...
Get USB UUID
     ...
     Write  ls -la /dev/disk/by-id/ | grep ${usbdev}-origin | tr -s " " |
...
USB Install
     Write  kexec -l /var/petitboot/mnt/dev/${usbdev}/ppc/ppc64/vmlinuz
     ... --initrd /var/petitboot/mnt/dev/${usbdev}/ppc/ppc64/initrd.img
     ... --append="ro inst.stage2=hd:UUID=${USBUUID} rd.dm=0 rd.md=0 nodmraid
     ... console=hvc0 ifname=net0:${mac} ip=${ip}::${gateway}:${net}:
     ... ${hostname}:net0:off nameserver=${dns} inst.ks=ubi0:/home/root/wsp/
     ... ks.cfg"
...
DVD Install
     ...
     ... console=hvc0 ifname=net0:${mac} ip=${ip}::${gateway}:${net}:
     ... ${hostname}:net0:off nameserver=${dns}"
...
Request ftp3 credendtials
     ...
     ${MyRepo}=  Set variable  ftp://${USER}:${PASSWD}@${ftp}/redhat/release_c
     ... ds/RHEL-ALT-7.4-GA/Server/ppc64le/os/
     ...
     Write  kexec -l vmlinuz --initrd initrd.img --append="root=live:${MyRepo}
     ... /LiveOS/squashfs.img repo=${MyRepo} rd.dm=0 rd.md=0 nodmraid console=
     ... hvc0 ifname=net0:${mac} ip=${ip}::${gateway}:${net}:${hostname}:net0:
