...
*** Test Cases ***
Get Storage Info and SCP uCode Files
    [Documentation]  Get storage firmware level type and size and scp ucode
    ...  files.
    [Tags]  Get_Storage_Info_and_SCP_uCode_Files
...
Reboot OS and Wait for Completion
    [Documentation]  Reboots the OS and waits until process is complete.
    [Tags]  Reboot_OS_and_Wait_for_Completion
...
Verify uCode Update
    [Documentation]  Verifies that the update was successful.
    [Tags]  Verify_uCode_Update
    Login To OS
    # Get firmware level.
