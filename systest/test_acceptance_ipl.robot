*** Settings ***
Documentation  Make a general hardware stress in a partition with the whole
...  resources available.
...
Run Soft Bootme
    [Documentation]  Run a soft bootme with a period of an hour.

    ${bootme_cmd}=  Catenate  htxcmdline -bootme on mode:soft period:3
    ${output}=  Execute Command On OS  ${bootme_cmd}
    Should Contain  ${output}  bootme on is completed successfully

Shutdown Bootme
    [Documentation]  Stop the bootme process.

    ${output}=  Execute Command On OS  htxcmdline -bootme off
