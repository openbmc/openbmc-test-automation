...
...                       -p ${OPENBMC_PASSWORD} -S Always -d ${EXECDIR}${/}Redfish-Usecase-Checkers-logs${/}
...
Test BMC Redfish Account Management
    ...
    ${output}=  Shell Cmd  cat ${EXECDIR}${/}Redfish-Usecase-Checkers-logs${/}results.json
    Log  ${output}

    ${json}=  OperatingSystem.Get File    ${EXECDIR}${/}Redfish-Usecase-Checkers-logs${/}results.json
