...
Redfishtool Create User
    ...
    [Arguments]  ${user_name}  ${password}  ${roleID}  ${Enabled}  ${expected_error}=""
    ...
    # Enabled        Enabled attribute of (e.g. true or false).
    # expected_error Expected error optionally provided in testcase (e.g. 401 / authentication error, etc. )

    ${Data}=   Set Variable   '{"UserName":${user_name},"Password":${password},"RoleId":${roleId},"Enabled":${Enabled}}'
    ${cmd_output}=  Redfishtool Post  ${Data}  /redfish/v1/AccountService/Accounts  ${root_cmd_args}  ${expected_error}
...
Redfishtool Update User Role
    ...
    {cmd_output}=  Redfishtool Patch  ${Data}  /redfish/v1/AccountService/Accounts/${user_name}  ${root_cmd_args}  ${expected_error}
...
Redfishtool Delete User
    ...
    # Description of argument(s):
    # user_name  user The user name (e.g. "test", "robert", etc.).
    # expected_error  Expected error optionally provided in testcase (e.g. 401 / authentication error, etc. ).
...
Redfishtool Get
    ...
    #${cmd_output}=  Run  ${cmd_args} GET ${uri}i
...
Redfishtool Post
    ...
    # Description of argument(s):
    # payload         Payload with POST operation (e.g. data for user name, password, role, enabled attribute)
