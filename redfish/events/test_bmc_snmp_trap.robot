...
Verify No SNMP Trap Sent After Deleting SNMP Manager
    ...
    # Configure SNMP manager and verify a trap is received to confirm the setup works.
    Configure SNMP Manager Via Redfish  ${SNMP_MGR1_IP}  ${SNMP_DEFAULT_PORT}  ${HTTP_CREATED}
    Start SNMP Manager
    ...
    # Now delete the SNMP manager — traps should no longer be dispatched.
    Delete SNMP Manager Via Redfish  ${SNMP_MGR1_IP}  ${SNMP_DEFAULT_PORT}

    # Start the listener again to capture any spurious traps after deletion.
    Start SNMP Manager

    BMC Execute Command  ${CMD_INTERNAL_FAILURE}

    SSHLibrary.Switch Connection  snmp_server
    Sleep  5s  # 5 s covers observed BMC trap-dispatch latency; increase if spurious traps are missed.
    ${snmp_listen_out_after}=  Read  delay=1s
    SSHLibrary.Execute Command  sudo killall snmptrapd

    # Verify no SNMP trap was sent after the manager was deleted.
