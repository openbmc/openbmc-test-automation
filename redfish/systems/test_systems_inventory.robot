...
Get GPU Inventory Via Redfish
    [Documentation]  Get the number of GPUs that are functional and enabled.
    [Tags]  Get_GPU_Inventory_Via_Redfish

    ${total_num_gpus}=  Set Variable  ${0}

    :FOR  ${gpu_uri}  IN  @{gpu_uris}
        ${is_functional}=  Check If FRU Is Functional  ${gpu_uri}
        ${total_num_gpus}=  Evaluate  $total_num_gpus + $is_functional
    END

    #  A system may have 0-6 GPUs.  For now, only print the total.
    Rprint Vars  total_num_gpus
