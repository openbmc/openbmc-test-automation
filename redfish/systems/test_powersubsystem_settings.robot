...
Verify Allowable Power Mode Settings Switch
    [Documentation]  Check that allowable power modes are set successfully at runtime. 
...
Verify State Of PowerSubsystem Power Supplies 
    [Documentation]  Verify the state of the system's power supplies is ok and enabled.
    [Tags]  Verify_State_Of_PowerSubsystem_Power_Supplies
...
Verify PowerSubsystem Efficiency Percent For All Power Supplies
    [Documentation]  Verify the efficiency percent for all power supplies.
    [Tags]  Verify_PowerSubsystem_Efficiency_Percent_For_Power_Supplies
