...
Verify Power Supplies Efficiency Percentage
    ...
    [Tags]  Verify_Power_Efficiency_Percentage
...
Verify Efficiency Percent Value
    [Documentation]  Verify the efficiency perfect for powersupplies is valid.
    [Arguments]  ${record_type}  ${redfish_uri}  ${reading_type}
    ...
    ${values}=  Redfish.Get Attribute  ${redfish_uri}  ${record_type}
    Rprint Vars  values
    ...
    ${invalid_values}=  Filter Struct  ${values}  [('{$reading_type}', '^90$')]  regex=1  invert=1
