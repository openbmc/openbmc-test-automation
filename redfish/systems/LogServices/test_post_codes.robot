...
Test PostCode Log Perisistency After BMC Reboot
    ...
    [Tags]  Test_PostCode_Log_Perisistency_After_Host_Reboot
    [Setup]  Redfish.Login

    # Ensure log entries are available before reboot
    Populate PostCode Logs Incase No Prior Entries Available
...
Test Clear Post Code Log Action
    ...
    # Ensure log entries are available before clear action
    Populate PostCode Logs Incase No Prior Entries Available
...
Get Boot Count Of Last PostCode Entry
    [Documentation]  Get the latest boot count from post code log entry
    ...  e.g if the log entry has ID "B2-1000", latest boot count "2" is returned
