...
Library          ../../data/platform_variables_import.py
...
Kill The List Of Services And Expect Killed Service Gets Restarted
    [Documentation]  Kill the given services and expect again services gets restarted automatically.
    [Tags]  Kill_The_List_Of_Services_And_Expect_Killed_Service_Gets_Restarted

    # Create an list of services in respective server model python file 
    # like romulus.py, witherspoon.py on openbmc-test-automation/data directory etc.
    # Example of creating an list of services in their respective server model python file
    # SERVICES = {
    ...
    ...    Fail  msg=All the given services has incorrect auto-restart policy.
    ...
    # If service process id was 0 or service was not in active state then those services will gets
    # appended to service_not_started list.
    # only services which has process id and in active state were gets killed and checks whether
    # those services were automatically restarted and was in active state only.
    ...
    FOR  ${service}  IN  @{auto_restart_policy_always_services}
      ...
      Continue For Loop If  ${main_pid_status} == False or ${active_state_status} == False
      ...
      ...  msg=Old process id was again mapped to ${service} service after service gets restarted.
    ...
    ...    Run Keywords  Log  ${service_not_started}  AND  Fail  msg=Services were either not started or not in active state by default.
    ...  ELSE IF  ${incorrect_services} != ${length_services} and ${length_service_not_started} != 0
    ...    Run Keywords  Log  ${service_not_started}  AND  Fail  msg=Listed services either not started or not in active state by default.
