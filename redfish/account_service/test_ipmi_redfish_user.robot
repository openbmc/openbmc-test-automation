...
Create IPMI User And Verify Login Via Redfish
    ...
    ${username}  ${userid}=  Create IPMI User With Given Settings
...
Create IPMI User With Given Settings
    [Documentation]  Create IPMI user with given password, privilege and IPMI
    ...  messaging settings.
    [Arguments]  ${password}  ${userpriv}

    # Description of argument(s):
    # password      Password to be assigned for the user.
    # userpriv      Privilege level for the user (e.g. "1", "2", "3",  etc.).
    ...
    Set Channel Access  ${random_userid}  ipmi=on privilege=${userpriv}
