...
Verify AccountService Unsupported Methods
    ...
    Redfish.Delete  ${REDFISH_ACCOUNTS_URI}
    ...  valid_status_codes=[${HTTP_METHOD_NOT_ALLOWED}]
...
Verify History Password For Redfish User
    ...
    VAR  &{payload}  UserName=${username}  Password=HistUserPwd1  RoleId=${role_id}  Enabled=${True}

    Redfish.Post  ${REDFISH_ACCOUNTS_URI}  body=&{payload}
    ...  valid_status_codes=[${HTTP_CREATED}]

    # Change password
    Redfish.Patch  ${REDFISH_ACCOUNTS_URI}${username}  body={'Password': 'HistUserPwd2'}
    ...  valid_status_codes=[${HTTP_OK}, ${HTTP_NO_CONTENT}]

    # Try to change password to previous one
    Redfish.Patch  ${REDFISH_ACCOUNTS_URI}${username}  body={'Password': 'HistUserPwd1'}
    ...  valid_status_codes=[${HTTP_OK}, ${HTTP_NO_CONTENT}]

    # Verify login.
    Redfish.Logout
    Redfish.Login  ${username}  HistUserPwd1
    Redfish.Logout
    Redfish.Login
    Redfish.Delete  ${REDFISH_ACCOUNTS_URI}${username}
...
Verify Minimum Password Length For Redfish User
    ...
    VAR  &{payload}  UserName=${username}  Password=UserPwd  RoleId=${role_id}  Enabled=${True}
    Redfish.Post  ${REDFISH_ACCOUNTS_URI}  body=&{payload}
    ...  valid_status_codes=[${HTTP_BAD_REQUEST}]
    ...
    Redfish.Patch  ${REDFISH_ACCOUNTS_URI}${username}  body={'Password': 'UserPwd'}
    ...  valid_status_codes=[${HTTP_BAD_REQUEST}]

    # Change to a valid password.
    Redfish.Patch  ${REDFISH_ACCOUNTS_URI}${username}  body={'Password': 'UserPwd1'}
    ...  valid_status_codes=[${HTTP_OK}, ${HTTP_NO_CONTENT}]

    # Verify login.
    Redfish.Logout
    Redfish.Login  ${username}  UserPwd1
    Redfish.Logout
    Redfish.Login
    Redfish.Delete  ${REDFISH_ACCOUNTS_URI}${username}
