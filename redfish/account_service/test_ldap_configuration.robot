...
Verify LDAP Config Update With Invalid URL Scheme ldapt
    ...
    # Confirm config is intact and LDAP user should still be able to login.
    ${login_status}=  Run Keyword And Return Status
    ...  Redfish.Login  ${LDAP_USER}  ${LDAP_USER_PASSWORD}
    Should Be True  ${login_status}
    ...  msg=LDAP login failed and ldapt:// patch may have corrupted config.
    Redfish.Logout
...
Verify Privilege Change By Local Admin When LDAP Is Unreachable
    ...
    Redfish Create User  ${test_local_user}  ${test_user_password}  ${privilege_admin}  ${True}  ${True}
    ...
    # Establish admin session: Verify User Login And Logout ends with no active session.
    Redfish.Login
