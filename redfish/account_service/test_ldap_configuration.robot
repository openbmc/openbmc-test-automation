...
Verify Authorization With Null Privilege
    [Documentation]  Verify that LDAP user authorization should fail with
    ...  empty privilege.
...
Verify LDAP Login With Invalid Data
    [Documentation]  Verify that LDAP login should fail with Invalid LDAP
    ...  data and right LDAP user.
...
Verify LDAP Config Creation Without BASE_DN
    [Documentation]  Verify that LDAP login should fail when LDAP
    ...  configuration created without BASE_DN.
...
Verify LDAP Authentication Without Password
    [Documentation]  Verify that LDAP user authentication should fail without
    ...  LDAP user password.
