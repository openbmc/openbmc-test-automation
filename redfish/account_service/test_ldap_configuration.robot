...
Update LDAP Configuration with LDAP User Role And Group
    ...
    ${localroleremotegroup}=  Create Dictionary  LocalRole=${group_privilege}  RemoteGroup=${group_name}
    ${remoterolemapping}=  Create List  ${localroleremotegroup}
    ${ldapdata}=  Create Dictionary  RemoteRoleMapping=${remoterolemapping}
    ${payload}=  Create Dictionary  ${ldap_type}=${ldapdata}
