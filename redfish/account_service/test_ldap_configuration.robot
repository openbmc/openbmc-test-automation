...
Verify Privilege Change Via Unreachable LDAP Falls Back To Local Admin
    [Documentation]  Create test user with admin privilege from local admin user, attempt to
    ...  change privilege from Admin to ReadOnly using LDAP user. Since LDAP is not reachable,
    ...  expect an error, then change the privilege via local admin user and verify.
    [Tags]  Verify_Privilege_Change_Via_Unreachable_LDAP_Falls_Back_To_Local_Admin
    [Teardown]  Run Keywords  Run Keyword And Ignore Error  Redfish.Login  AND
    ...  Run Keyword And Ignore Error  Create LDAP Configuration  AND
    ...
    # Restore admin session before reconfiguring LDAP.
    Redfish.Login
    ...
    # Verify LDAP server is now unreachable.
    Verify LDAP Is Unreachable

    # Attempt privilege change via LDAP user — expect failure since LDAP is unreachable.
    ${status}=  Run Keyword And Return Status
    ...  Redfish.Login  ${LDAP_USER}  ${LDAP_USER_PASSWORD}
    Should Be Equal  ${status}  ${False}
    ...  msg=LDAP user should not be able to login when LDAP is unreachable.

    # Restore local admin session for the fallback privilege change.
    Redfish.Login

    # Fallback: change privilege from Administrator to ReadOnly using local admin.
    Redfish.Patch  ${REDFISH_ACCOUNTS_URI}${test_local_user}
...
Verify Local User Created By LDAP Admin Disabled By Local Admin And Enabled By Service User
    ...
    # Re-establish admin session after the failed login attempt above.
    Redfish.Login
...
Verify User Role
    ...
    # Description of argument(s):
    # username       The username of the user account.
    # expected_role  The expected role (e.g., Administrator, ReadOnly).
...
Cleanup Local User And Restore Session
    [Documentation]  Cleanup test user account and restore admin session.
    [Arguments]  ${username}

    # Description of argument(s):
    # username  The username of the local user account to delete.
...
Creator User Creates Local User And LDAP User Changes Privilege
    [Documentation]  Creator user creates a local test user, LDAP admin changes the privilege
    ...  and verifies the test user can login with the updated privilege.
