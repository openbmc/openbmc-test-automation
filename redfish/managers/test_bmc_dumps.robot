...
Verify Dump Persistency On Dump Service Restart
    ...
    Redfish Power Off    stack_mode=skip
    Create User Initiated BMC Dump Via Redfish
    ${dump_entries_before}=    redfish_utils.get_member_list
    ...
    ${dump_entries_after}=    redfish_utils.get_member_list
    ...    /redfish/v1/Managers/${MANAGER_ID}/LogServices/Dump/Entries
...
Verify Maximum BMC Dump Creation
    ...
    FOR    ${n}    IN RANGE    0    ${MAX_DUMP_COUNT}
        ...
        IF    ${dump_space} >= (${BMC_DUMP_TOTAL_SIZE} - ${BMC_DUMP_MIN_SPACE_REQD})
            BREAK
        END
    END
...
Download BMC Dump
    ...
    ${ret}=    GET On Session
    ...    openbmc
    ...    /redfish/v1/Managers/${MANAGER_ID}/LogServices/Dump/Entries/${dump_id}/attachment
    ...    headers=${headers}
