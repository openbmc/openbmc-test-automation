...
Suite Setup Execution
    ...
    ${eth1_initial_ipv4_addressorigin_list}  ${eth1_initial_ipv4_addr_list}=  Get Address Origin List And IPv4 or IPv6 Address  IPv4Addresses  ${2}
    ${eth1_initial_ipv6_addressorigin_list}  ${eth1_initial_ipv6_addr_list}=  Get Address Origin List And IPv4 or IPv6 Address  IPv6Addresses  ${2}
...
Verify All The Addresses Are Intact
    ...
    # Description of argument(s):
    # ip_address_type                  Type of SLAAC or DHCPv6 address to be checked.
    # channel_number                   Channel number 1(eth0) or 2(eth1).
    ...
    @{ipv6_addressorigin_list}  ${ipv6_linklocal_addr}=  Get Address Origin List And Address For Type  LinkLocal  ${channel_number}

    # IPv4 and IPv6 addresses must remain intact.
    ${ipv4_addressorigin_list}  ${ipv4_addr_list}=  Get Address Origin List And IPv4 or IPv6 Address  IPv4Addresses  ${channel_number}
    ${ipv6_addressorigin_list}  ${ipv6_addr_list}=  Get Address Origin List And IPv4 or IPv6 Address  IPv6Addresses  ${channel_number}
...
Set And Verify SLAAC Property On Both Interfaces
    ...
    [Arguments]  ${slaac_value_1}  ${slaac_value_1_2}
    ...
    IF  "${slaac_value_1}" == "${True}"
         @{ipv6_addressorigin_list}  ${ipv6_slaac_addr}=  Get Address Origin List And Address For Type  SLAAC  ${1}
    ELSE
        ${ipv6_addressorigin_list}  ${ipv6_addr_list}=  Get Address Origin List And IPv4 or IPv6 Address  IPv6Addresses  ${1}
        Should Not Contain  ${ipv6_addressorigin_list}  SLAAC
    END
    IF  "${slaac_value_2}" == "${True}"
         @{ipv6_addressorigin_list}  ${ipv6_slaac_addr}=  Get Address Origin List And Address For Type  SLAAC  ${2}
    ELSE
        ${ipv6_addressorigin_list}  ${ipv6_addr_list}=  Get Address Origin List And IPv4 or IPv6 Address  IPv6Addresses  ${2}
        Should Not Contain  ${ipv6_addressorigin_list}  SLAAC
    END
