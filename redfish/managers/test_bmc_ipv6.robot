...
Verify Coexistence Of All IPv6 Type Addresses Combination On BMC
    [Documentation]  Verify coexistence of all ipv6 type addresses combination on BMC.
    [Tags]  Verify_Coexistence_Of_All_IPv6_Type_Addresses_Combination_On_BMC
    [Setup]  Run Keywords  Get The Initial DHCPv6 Setting On Each Interface  ${1}
    ...      AND  Get The Initial SLAAC Setting On Each Interface  ${1}
    [Teardown]  Run Keywords  Set And Verify DHCPv6 Property  ${dhcpv6_channel_1}  ${1}
    ...         AND  Set SLAAC Configuration State And Verify  ${slaac_channel_1}  [${HTTP_OK}]  ${1}

    @{address_types}=  Create List  LinkLocal  Static  DHCPv6  SLAAC

    #Verify coexistence of all ipv6 type addresses combination on BMC.
    FOR  ${type1}  IN  @{address_types}
        FOR  ${type2}  IN  @{address_types}
            IF  '${type1}' != '${type2}'

                #Configure the first address type.
                IF  '${type1}' == "Static"
                    Configure IPv6 Address On BMC  ${test_ipv6_addr}  ${test_prefix_length}
                ELSE IF  '${type1}' == "DHCPv6"
                    Set And Verify DHCPv6 Property  Enabled
                ELSE IF  '${type1}' == "SLAAC"
                    Set SLAAC Configuration State And Verify  ${True}
                END

                #Configure the second address type.
                IF  '${type2}' == "Static"
                    Configure IPv6 Address On BMC  ${test_ipv6_addr}  ${test_prefix_length}
                ELSE IF  '${type2}' == "DHCPv6"
                    Set And Verify DHCPv6 Property  Enabled
                ELSE IF  '${type2}' == "SLAAC"
                    Set SLAAC Configuration State And Verify  ${True}
                END
