...
Enable And Verify DHCPv6 Property On Eth1 When DHCPv6 Property Enabled On Eth0
    [Documentation]  Verify DHCPv6 on eth1 whrn DHCPv6 property is enabled on eth0.
    ...
    ${CHANNEL_NUMBER}=  Set Variable  ${1}
    ${ethernet_interface}=  Set Variable  ${active_channel_config['${CHANNEL_NUMBER}']['name']}
    ${resp}=  Redfish.Get  ${REDFISH_NW_ETH_IFACE}${ethernet_interface}
    ${dhcpv6_verify}=  Get From Dictionary  ${resp.dict}  DHCPv6
    ${dhcpv6_setting_before_1}=  Set Variable  ${dhcpv6_verify['OperatingMode']}

    ${CHANNEL_NUMBER}=  Set Variable  ${2}
    ${ethernet_interface}=  Set Variable  ${active_channel_config['${CHANNEL_NUMBER}']['name']}
    ${resp}=  Redfish.Get  ${REDFISH_NW_ETH_IFACE}${ethernet_interface}
    ${dhcpv6_verify}=  Get From Dictionary  ${resp.dict}  DHCPv6
    ${dhcpv6_setting_before_2}=  Set Variable  ${dhcpv6_verify['OperatingMode']}
    ...
    Set And Verify DHCPv6 Property  ${dhcpv6_setting_before_1}  ${1}
    Set And Verify DHCPv6 Property  ${dhcpv6_setting_before_2}  ${2}
