...
Suite Setup Execution
    ...
    Should Be Equal  ${status}  ${True}  msg=${OPENBMC_HOST} not configured with eth0 interface.

    ${status}=   Run Keyword And Return Status  Ping Host  ${OPENBMC_HOST_1}
    Should Be Equal  ${status}  ${True}  msg=${OPENBMC_HOST} not configured with eth1 interface.
