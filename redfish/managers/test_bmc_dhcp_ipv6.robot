...
Get DHCPv6 Address And Verify Connectivity
    ...
    Connect To IPv6 Host Via SSH  ${ipv6_dhcpv6_addr}  ${server_connected}
...
Check Ping6 Status And Verify
    ...
    [Arguments]  ${OPENBMC_HOST_IPv6}  ${server_connected}=${True}

    # Description of argument(s):
    # OPENBMC_HOST_IPv6   IPv6 address to Connectivity.
    # server_connected    Flag to indicate if already connected to server (True/False).
    #                     Default is True, which means it will not connect to server.
...
Connect To IPv6 Host Via SSH
    ...
    # server_connected    Flag to indicate if already connected to server (True/False).
    #                     Default is True, which means it will not connect to server first.
