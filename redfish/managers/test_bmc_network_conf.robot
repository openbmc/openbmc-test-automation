...
Verify NameServers Field Cannot Be Directly Modified Via Redfish Patch Operation
    [Documentation]  Verify NameServers field cannot be directly modified via redfish
    ...  Patch operation.
    [Tags]  Verify_NameServers_Field_Cannot_Be_Directly_Modified_Via_Redfish_Patch_Operation
...
Verify IPv4 Link Local Absent When Static Or DHCP Configured
    [Documentation]  Verify IPv4 link-local is not present when Static or DHCP IPv4
    ...  is configured on the default Ethernet interface (channel ${CHANNEL_NUMBER}).
    [Tags]  Verify_IPv4_Link_Local_Absent_When_Static_Or_DHCP_Configured

    ${ipv4_origin_list}  ${_}=
    ...  Get Address Origin List And IPv4 or IPv6 Address  IPv4Addresses

    ${has_static_or_dhcp}=  Evaluate
    ...  any(o in ('Static', 'DHCP') for o in $ipv4_origin_list)
    Skip If  not ${has_static_or_dhcp}
    ...  Skipping: interface has no Static or DHCP IPv4 - prerequisite not met.
