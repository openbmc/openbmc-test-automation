...
Verify Adding New Static IPv4 With New Gateway On Eth0 Fails
    [Documentation]  Verify adding new static IPv4 address with new gateway on eth0 fails.
    [Tags]  Verify_Adding_New_Static_IPv4_With_New_Gateway_On_eth0_Fails

    ${old_gateway}=  Get BMC Default Gateway

    # Verify new gateway is not same as default gateway.
    Should Not Be Equal  ${test_ipv4_gateway}  ${old_gateway}
