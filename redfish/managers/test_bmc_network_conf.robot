...
Add IPv4Address
    ...
    ${status}=  Run Keyword And Return Status  Patch An Element  IPv4Addresses
    ...  ${patch_list}  ${REDFISH_NW_ETH0_URI}  ${expected_status}

    # Note: Modification to network config takes time to reflect on CLI
    Run Keyword If  '${expected_status}' == ${HTTP_OK}
    ...  Wait Until Keyword Succeeds  2min  10sec  Verify IP On BMC  ${ip}
    ...  ELSE  Wait Until Keyword Succeeds  2min  10sec
    ...  Verify No IP On BMC  ${ip}
...
Delete IPv4Address
    ...
    ${ip_found}=  Set Variable  False
    
    @{network_configurations}=  Get Network Configuration
    
    : FOR  ${network_configuration}  IN  @{network_configurations}
    \    ${ip_found}=  Set Variable If
         ...  '${network_configuration['Address']}' == '${ip}'  True
    \    Run Keyword If  '${ip_found}' == 'True'
    ...
    # Modification to network config takes time to reflect on CLI
    # So, Wait Until Keyword Succeeds is used
    Run Keyword If  '${ip_found}' == 'True'
    ...  Run Keywords
...
Validate Network Config On BMC
    ...
    # Note: Network configs need some settling period after IP modification
    : FOR  ${network_configuration}  IN  @{network_configurations}
