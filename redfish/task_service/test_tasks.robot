...
Test Validity Of Generated Task Instance And Task State
    ...
    ${task_id}  ${resp_obj}=  Generate Task Instance And Return Task Id
...
Verify Task Persistency After BMC Reboot
    [Documentation]  Verify task collection persistency after BMC reboot.
    [Tags]  Verify_Task_Persistency_After_BMC_Reboot
...
Generate Task Instance And Return Task Id
    ...
    IF  '${task_type}' == 'bmc_dump'
        ${task_id}  ${resp}=  Create BMC User Dump And Return Task Id
    END
    Return From Keyword  ${task_id}  ${resp}


Wait For Task Completion
    [Arguments]  ${task_id}  ${expected_completion_state}  ${retry_max_count}=300
    ...  ${check_state}=${FALSE}
    ...
    #                             to be monitored.
    # expected_completion         the task state which is to be considered as the
    ...
    FOR  ${retry}  IN RANGE  ${retry_max_count}
        ...
        Exit For Loop If
        ...  '${resp["TaskState"]}' in ${expected_completion_state}
...
Generate Task Instance And Wait For Completion
    ...
    ${task_id}  ${resp}=  Generate Task Instance And Return Task Id  ${task_type}
    Wait For Task Completion  ${task_id}  ${allowed_task_completion_state}
