...
REST Logging Interface Read Should Be A SUCCESS For Authorized Users
    ...
    ## Redfish.Get is not fetching the body { 'data': ....}, hence REST way is used.
    ${resp}=  OpenBMC Get Request  /xyz/openbmc_project/logging

    ${resp_output}=  evaluate  json.loads('''${resp.content}''')  json
