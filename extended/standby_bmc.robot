...
...               to bring the BMC to a stable and HOST in off state.
...               For our purposes, a stable state is defined as:
...
Get to Stable State
    ...
    ...                4. Get BMC in Ready state if its not in this state
    ...                5. Get Host in Off state if its not in this state
    ...                6. Update restore policy
    ...
    ${l_hostoff}=   Run Keyword And Return Status
    ...    Is Host Off

    Run Keyword If  '${l_hostoff}' == '${False}'
