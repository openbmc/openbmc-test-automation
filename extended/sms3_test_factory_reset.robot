...
Resource  ../lib/boot/reboots.robot
...
Verify Network Factory Reset
    [Documentation]  Network actory reset the system and verify if BMC is online.
    [Tags]  Verify_Network_Factory_Reset

    # Network factory reset erases user config settings which incldes IP, netmask
    ...
    Should Be Equal  ${status}  False  msg=Network factory reset failed.
...
Verify Host Factory Reset
    [Documentation]  Host factory reset the system and verify if it clears
    ...
    Log To Console  HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
    ...
    ${cmd_output}=  Execute Command On BMC  df
    Should Not Contain  ${cmd_output}  pnor-rw  msg=Host factory reset failed.
    Should Not Contain  ${cmd_output}  pnor-prsv  msg=Host factory reset failed.

Verify BMC Factory Reset
    [Documentation]  BMC factory reset the system and verify if it clears
    ...
    ${cmd_output}=  Execute Command On BMC  df
    Should Not Contain  ${cmd_output}  cow  msg=BMC factory reset failed.
    Should Not Contain  ${cmd_output}  mtdblock5  msg=BMC factory reset failed.
