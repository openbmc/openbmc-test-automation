...
###############################################################################
def get_software_state(version_id=None):

    r"""
    Get the current software state of the ${version_id}.
    Includes Activation, RequestedActivation, Version and Purpose.

    Description of argument(s):
    version_id  The version id of which we want the software states.
    """

    version_id = gp.get_var_value(version_id, "")
    if not gv.valid_value(version_id):
        return False

    status, ret_values =\
        keyword.run_key("Read Properties  " + var.SOFTWARE_VERSION_URI +
                        str(version_id))
    return ret_values
...
def wait_for_activation_state_change(version_id, initial_state):
    ...
    retry = 0
    while (retry < 20):
        current_state = (get_software_state(version_id))["Activation"]
...
def get_pnor_version(file_path):
    ...
    keyword.run_key_u("Open Connection And Log In")
    status, ret_values =\
            keyword.run_key("Execute Command On BMC  "
            "grep \"extended_version=\" " + file_path);
