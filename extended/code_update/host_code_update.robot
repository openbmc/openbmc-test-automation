...
Test Boot With No VPD Cache
    ...
    ${vpd_file_count}  ${stderr}  ${rc}=  BMC Execute Command
    ...  ls ${cache_files_dir_path} | grep VPD -c
    # Typically, vpd files = "CVPD DJVPD MVPD NVRAM".

    Run Keyword If  ${vpd_file_count} != ${3}  Fatal Error
    ...  msg=No VPD files at ${cache_files_dir_path}.

    # Delete the *VPD* files.
    ${vpd_files}  ${stderr}  ${rc}=  BMC Execute Command
    ...  rm -F ${xxx_dir_path}*VPD*
    ...
    Run Keyword If  ${vpd_file_count} != ${3}  Fatal Error
    ...  msg=After power-on, no VPD files at ${cache_files_dir_path}.
