...
Verify Host Factory Reset
    ...
    ${cmd_output}=  Execute Command On BMC  ls /media/pnor-prsv
...
Verify BMC Factory Reset
    ...
    ${cmd_output}=  Execute Command On BMC  df
    Should Contain  ${cmd_output}  var  msg=Host factory reset failed.

    # Check BMC read-write and preserved files are deleted.

    ${cmd_output}=  Execute Command On BMC  ls /media/var
