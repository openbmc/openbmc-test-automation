...
Upload Image Via REST
    ...
    ${last_file}=  Get Latest File  ${UPLOAD_DIR_PATH}
    OperatingSystem.File Should Exist  ${IMAGE_FILE_PATH}
    ${image_data}=  OperatingSystem.Get Binary File  ${IMAGE_FILE_PATH}
    Upload Post Request  /upload/image  data=${image_data}
    ${upload_file}=  Get Latest File  ${UPLOAD_DIR_PATH}
    Should Not Be Equal  ${last_file}  ${upload_file}
    ${ret}=  Verify Upload Activation State  ${upload_file}
...
Upload Image Via REST With Name
    ...
    OperatingSystem.File Should Exist  ${IMAGE_FILE_PATH}
    ${image_data}=  OperatingSystem.Get Binary File  ${IMAGE_FILE_PATH}
    Upload Put Request  /upload/image/testimage   data=${image_data}
    ${upload_file}=  Get Latest File  ${UPLOAD_DIR_PATH}
    Should Not Be Equal  ${last_file}  ${upload_file}
    ${ret}=  Verify Upload Activation State  ${upload_file}
    Should Be True  True == ${ret}
