...
The objective of OpenBMC FVT is to verify that the hardware and firmware perform
according to architectural specifications.  This is done through both black box
and white box testing.

The goal of functional verification tests is to identify and fix problems with
the OpenBMC based servers. These functionality tests are focus on the basic
hardware and firmware(OpenBMC) to ensure that code development and job
execution functions meet the essential requirements for working on Intel /
Power / AMD based servers.
...
##  Functional Verification Test (FVT) Pre-requisites

 - Test target machine must be POWER/Intel chip based server to test with both
...
Ensure that function implementation meets architectural specifications as
...
For examaple, run HTX on host OS and do BMC reset functional testing.

## Functional Verification Test Scope

Any newly introduced OpenBmc feature falls within the scope of FVT.
That includes any features from the various supported interfaces such as
Redfish,REST,IPMI with IPMI 2.0 support, GUI.
...
within the OpenBMC FVT test scope.

Features currently in OpenBMC FVT scope:
...
Features which are related to testing using OpenBMC command like busctl,
systemctl etc. are considered as out of scope.
...
to formally begin:
 ...
 - The FTC result tracking mechanism is setup (execution records defined).
