...
To create and install CA signed server certificate, we need to follow below steps:
...
2. Create root CA certificate using above private key.
...
1. Create CSR request file(csr_file.json) with all below fields.
...
2. Generate CSR request using below Redfish command.
...
1. Now use BMC generated CSR request(device.csr) to generate CA signed certificate(device.crt)
```
openssl x509 -req -in device.csr -CA rootCA.pem -CAkey rootCA.key -CAcreateserial -out device.crt -days 500 -sha256
```


2. Now create json file(certificate.json) with above generated CA signed certificate file (device.crt).
...
**Install CA signed server certificate**
 ...
 Replace server certificate using above json file with CA signed certificate details(certificate.json)
