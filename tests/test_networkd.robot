*** Settings ***
Documentation       This suite will verifiy the Network Configuration Rest
...                 Interfaces
...
${NW_INTERFACE}      /org/openbmc/NetworkManager/Interface/
...
Get the Mac address
    ...
    ${resp}=   Call Method    ${NW_INTERFACE}   GetHwAddress   data=${args}
...
Get IP Address with invalid interface
    ...
    [Documentation]   This test case tries to get the ip addrees with the
    ...              invalid interface,Expectation is it should get error.
    [Tags]   network_test

    @{arglist}=   Create List   lo01
    ${args}=     Create Dictionary   data=@{arglist}
    ${resp}=    Call Method    ${NW_INTERFACE}   GetAddress4    data=${args}
...
Set the IP address on invalid Interface
                   lo01   1.1.1.1  255.255.255.0   1.1.1.1     error

   [Tags]   network_test
   [Template]    AddNetworkInfo
   [Documentation]    This test case tries to set the ip addrees with the
    ...               invalid interface,Expectation is it should get error

Set invalid IP address on the valid interface
                  eth0     ab.cd.ef.gh    255.255.255.0     1.1.1.1     error

   [Tags]   network_test
   [Template]    AddNetworkInfo
   [Documentation]    This test case tries to set the invalid ip addrees on the
    ...               interface, Expectation is it should get error.


Set IP address with invalid subnet mask
                  eth0  2.2.2.2   av.ih.jk.lm   1.1.1.1   error

   [Tags]   network_test
   [Template]    AddNetworkInfo
   [Documentation]   This test case tries to set the ip addrees on  the
   ...               interface with invalid subnet mask,Expectation is it
   ...               should get error.
...
Set empty IP address    eth0     ${EMPTY}   255.255.255.0   1.1.1.1   error
   ...
   [Tags]   network_test
   [Template]    AddNetworkInfo
   [Documentation]   This test case tries to set the NULL ip addrees on the
   ...               interface. Expectation is it should get error.
...
Set empty subnet mask    eth0   2.2.2.2    ${EMPTY}    1.1.1.1     error
   ...
   [Template]    AddNetworkInfo
   [Documentation]   This test case tries to set the ip addrees on the
   ...               interface with empty subnet mask,Expectation is
   ...               it should get error.
...
Set empty gateway    eth0    2.2.2.2    255.255.255.0     ${EMPTY}    error
   ...
   [Template]    AddNetworkInfo
   [Documentation]   This test case tries to set the ip addrees on the
   ...               interface with empty gateway,Expectation is it
   ...               should get error.


Test New IP Address On Valid Interface
    [Tags]   network_test
    [Documentation]   This test case sets the ip  on the interface and
    ...               validates that ip address has been set or not.
    ...               Expectation is the ip address should get added.

    Run Keyword And Ignore Error    Set IP address on valid Interface
...
Revert the last ip address change
    ...
    ...      Call Method    ${NW_INTERFACE}   SetAddress4    data=${args}
    ...
    ${isgatewayfound} =    Set Variable If   '${gateway}'=='${OLD_GATEWAY}'
    ...                    true    false
    ${isIPfound}=    Set Variable if    '${ipaddress}' == '${OLD_IP}'
    ...              true   false
...
Get IP Address type
    [Tags]   GOOD-PATH
    [Documentation]   This test case tries to set existing ipaddress address
    ...               and later tries to verify that ip address type is set
    ...               to static due to the operation.
    ...
    ${arglist}=    Create List
    ...      eth0    ${CURRENT_IP}   ${CURRENT_MASK}   ${CURRENT_GATEWAY}
    ${args}=     Create Dictionary   data=@{arglist}
    run keyword and ignore error
    ...    Call Method    ${NW_INTERFACE}   SetAddress4    data=${args}
    ...
    @{arglist}=   Create List   eth0
    ${args}=     Create Dictionary   data=@{arglist}
    ${resp}=    Call Method    ${NW_INTERFACE}   GetAddressType    data=${args}
    ${json} =   to json         ${resp.content}
...
Set valid Mac address     eth0     00:21:cc:73:91:dd   ok
    ...
    ...               This test case add the ip addresson the  interface and
    ...               validates that ip address has been added or not.
    ...               Expectation is the ip address should get added.
...
Get networkInfo from the interface
    ...
    [Documentation]   This keyword is used to match the given ip with the
    ...               configured one.
    ...               returns true if match successfull else false
    ...               eg:- Outout of getAddress4
    ...               NewFormat:-{"data": [ 2,25,"9.3.164.147","9.3.164.129"],
    ...                                "message": "200 OK","status": "ok"}
    ...               OldFormat:-
    ...               {"data": [[[2,25,0,128,"9.3.164.177"],
    ...                        [2,8,254,128,"127.0.0.1"]],"9.3.164.129"],
    ...                "message": "200 OK", "status": "ok"}
...
AddNetworkInfo
    ...
    ${args}=       Create Dictionary   data=@{arglist}
    ${resp}=       Call Method    ${NW_INTERFACE}   SetAddress4    data=${args}
    should not be equal as strings      ${resp.status_code}     ${HTTP_OK}
    ${json} =   to json         ${resp.content}
...
SetMacAddress_bad
    ...
    ${args}=       Create Dictionary   data=@{arglist}
    ${resp}=       Call Method   ${NW_INTERFACE}   SetHwAddress    data=${args}
    should not be equal as strings      ${resp.status_code}     ${HTTP_OK}
    ${json} =   to json         ${resp.content}
    should be equal as strings      ${json['status']}       ${result}
...
Set IP address on valid Interface
    ...
    ${valid}=   Run Keyword and Return Status    validateEnvVariables
    Return From Keyword If   '${valid}' == '${False}'
    ...
    ${isgatewayfound} =    Set Variable If   '${gateway}'=='${NEW_GATEWAY}'
    ...                    true    false
    ${isIPfound}=    Set Variable if    '${ipaddress}' == '${NEW_IP}'
    ...   true   false
    should be true   '${isIPfound}' == 'true' and '${isgatewayfound}' == 'true'
