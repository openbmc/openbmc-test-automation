...
Set BMC Time On Valid Config
    ...
    [Documentation]  Set BMC time on a valid configuration.
...
Set BMC Time On Invalid Config
    ...
    BOTH          NTP         error             Not Set             No Change
...
Set HOST Time On Valid Config
    ...
    BOTH          NTP         ok                No Change           Set
    SPLIT         Manual      ok                No Change           Set
    SPLIT         NTP         ok                No Change           Set
    [Documentation]  Set HOST time on a valid configuration.
...
Set BMC Time
    ...
    @{time_owner_date}=   Create List   BMC   ${setdate}
...
Set HOST Time
    ...
    @{time_owner_date}=   Create List   HOST   ${SYSTEM_TIME_VALID_EPOCH}
    ${data}=   create dictionary   data=@{time_owner_date}
    ${resp}=   openbmc post request
    ...        ${TIME_MANAGER_URI}action/SetTime   data=${data}
    ${jsondata}=   To Json    ${resp.content}
