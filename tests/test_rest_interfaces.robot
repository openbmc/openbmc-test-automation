...
Verify All REST Objects Are Accessible
    ...
    :FOR  ${uri}  IN  @{uri_list}
    \  ${resp1}=  Openbmc Get Request  ${uri}  quiet=${1}
    \  ${resp2}=  Openbmc Get Request  ${uri}/  quiet=${1}
    \  Run keyword if  '${resp1.status_code}' != '${HTTP_OK}'
    \  ...  Append To List  ${error_uri_list}  ${uri} : ${resp1.status_code}
    \  Run keyword if  '${resp2.status_code}' != '${HTTP_OK}'
    \  ...  Append To List  ${error_uri_list}  ${uri}/ : ${resp2.status_code}
