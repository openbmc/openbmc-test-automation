...
Test Error Persistency On Restart
    [Documentation]  Restart logging service and check if error logs exist.
...
Test Error Persistency On Reboot
    [Documentation]  Reboot BMC and check if error logs exist.
...
Clear Existing Error Logs
    [Documentation]  Clear existing error logs.

    ${resp}=  OpenBMC Get Request  ${BMC_LOGGING_ENTRY}${1}
    # If entry doesn't exist, return from keyword.  Otherwise, reboot the BMC
    # to clear the existing error logs.
