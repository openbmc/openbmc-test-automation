...
Set LED State On
    [Documentation]  Set state of given LED to on.
    [Arguments]  ${led_name}

    ${state}=  Set Variable  true
    ${state}=  Convert To Boolean  ${state}
    ${data}=  Create Dictionary  data=${state}

    ${resp}=  OpenBMC Put Request
    ...  ${LED_GROUP_URI}${led_name}/attr/Asserted  data=${data}

    ${jsondata}=  to JSON  ${resp.content}
    [Return]  ${jsondata['status']}

Set LED State Off
    [Documentation]  Set state of given LED to off.
    [Arguments]    ${led_name}
...
Setup The Suite
    ...
    ${resp}=  Read Properties  ${LED_GROUP_URI}
...
Get LED List
    ...
    ${list}=  Get Matches  ${LED_GROUPS}  regexp=^.*[0-9a-z_].${led_prefix}.*${led_suffix}
    ${new_list}=  Create List

    : FOR  ${element}  IN  @{list}
    \  ${element}=  Remove String  ${element}  ${LED_GROUP_URI}
