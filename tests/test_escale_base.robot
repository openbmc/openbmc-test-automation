...
Escale Power Setting Via REST And Verify In IPMI
    [Documentation]  Set power via REST and check using IPMI.
    [Tags]  Escale_Power_Setting_Via_REST_And_Verify_In_IPMI

    Set DCMI Power Limit Via REST  ${power_1700}
    ${power}=  Get DCMI Power Limit Via REST
    Should Be True  ${power} == ${power_1700}
    ...  msg=Powercap reading via REST after setting it via REST failed.
...
Escale Power Setting Via IPMI And Verify In REST
    [Documentation]  Set power via IPMI then check in REST.
    [Tags]  Escale_Power_Setting_Via_IPMI_And_Verify_In_REST
