...
Test IPMI Warm Reset
    ...
    ...  Reset BMC Via IPMI  warm   Sent warm reset command to MC
...
Test IPMI Cold Reset
    ...
    Repeat Keyword  ${LOOP_COUNT} times
    ...  Reset BMC Via IPMI  cold   Sent cold reset command to MC


*** Keywords ***

Reset BMC Via IPMI
    [Documentation]  Reset BMC via IPMI (eg. "warm/cold").
    [Arguments]  ${cmd}  ${value}

    # Description of argument(s):
    # cmd    Command string (eg. "warm", "cold").
    # value  Command output string (eg. "Sent warm reset command to MC").

    ${resp}=  Run IPMI Standard Command  mc reset ${cmd}
    Should Be Equal As Strings  ${resp}  ${value}
