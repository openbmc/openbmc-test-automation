...
Verify FRU Component
    [Documentation]  Verify IPMI FRU component info.
    ...  with REST.
    ...
    # Remove numeric value from component name. i.e. cpu0 to cpu.
    ${main_component_name}=
    ...  Remove String Using Regexp  ${component_name}  [0-9]

    # Get pre-defined set list from data/ipmi_fru_table.py
    # Example:
    # set(['Board Mfg Date', 'Board Mfg', 'Board Product', 'Board Serial']}
    @{field_list}=  Get From Dictionary  ${ipmi_fru_dict}  ${main_component_name}

    : FOR  ${element}  IN  @{field_list}
    \  Run Keyword If  '${element}' != '${EMPTY}'
    \  ...    Verify FRU Component Field  ${component_name}  ${element}


Verify FRU Component Field
    ...
    ${rest_name}  ${ipmi_name}=
    ...  Run Keyword If  '${field_name}' == 'Board Mfg Date'
    ...  Set Variable  BuildDate  Board Mfg Date
    ...  ELSE IF  '${field_name}' == 'Board Mfg'
...
Verify FRU Info
    ...
    \  Verify FRU Component  ${component}
