...
Verify Host Quiesce State
    ...
    ...  Put Current And Transition Host State  Off  Off
    ...  ELSE IF  '${host_state}' == 'Running'
    ...  Put Current And Transition Host State  Running  On
...
Put Current And Transition Host State
    [Documentation]  Put host in given current and transition state.
    [Arguments]  ${HOST_STATE}  ${HOST_TARNS_STATE}
    # Description of Arguments:
    # host_state  Expected host state.
    # host_trans_state  Expected transition host state.

    ${trans_state}=  Get Host Trans State
    ${state}=  Get Host State

    ${condition}=   Catenate  SEPARATOR=
    ...  '${host_state}' == '${state}' and${SPACE}
    ...  '${host_trans_state}' == '${trans_state}'

    Log to console  host state : ${host_state}
    Run Keyword If  ${condition}  Log  Host is already in expected state.
    ...  ELSE IF  '${host_state}' == 'Running' and '${host_trans_state}' == 'On'
    ...  Run Keywords  Initiate Host PowerOff  AND  Initiate Host Boot
    ...  ELSE IF  '${host_state}' == 'Off' and '${host_trans_state}' == 'Off'
    ...  Run Keywords  Initiate Host Boot  AND  Initiate Host PowerOff

    ${trans_state}=  Get Host Trans State
    Should Be Equal  ${host_trans_state}  ${trans_state}
