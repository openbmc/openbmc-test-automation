...
* Run IPMI tests:
    ...
    ```
    $ robot -v OPENBMC_HOST:x.x.x.x -v IPMI_COMMAND:External --argumentfile test_lists/witherspoon/skip_inband_ipmi tests/ipmi/
    ```

    Running only inband IPMI tests:
    ```
    $ robot -v OPENBMC_HOST:x.x.x.x -v OS_HOST:x.x.x.x -v OS_USERNAME:xxxx -v OS_PASSWORD:xxxx -v IPMI_COMMAND:Inband --argumentfile test_lists/witherspoon/skip_oob_ipmi tests/ipmi/
    ```
