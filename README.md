...
If running on a Python3.x level, use the corresponding `pip3` to install the
...
## Code Update ##
    ...
    Currently supported BMC and PNOR update format are UBI and non-UBI.
...
* Non-UBI Format *
    ...
    For host code update, download the system type *.pnor
    from https://openpower.xyz/job/openpower-op-build/ and run as follows:
