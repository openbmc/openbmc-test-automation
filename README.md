...
There are different set of test suite, tools and templates existing based on the usage.
...
```
    ...
    templates/
```
`tests`: directory contains the general test cases for OpenBMC stack functional verification.

`extended`: directory contains the use cases for boot testing, network testing, code update etc.
...
`templates`: directory contains sample coding guideline code examples and setup testing.
`
