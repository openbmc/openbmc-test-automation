...
parser.add_argument(
    ...
    default='logging',
    help='The type of notifications from websocket to monitor, either logging or dump.')
...
def on_message(websocket_obj, message):
    ...
    if monitoring_uri + '/entry' in message and 'Id' in message:
        qprint_timen('eSEL received over websocket interface.')
        qprint_timen("Closing websocket. Expect to receive 'NoneType' obj has no attrib 'connected'.")
        websocket_obj.close()
    else:
        if monitoring_uri + '/entry' in message and 'Size' in message:
            qprint_timen('dump notifiication received over websocket interface.')
            qprint_timen("Closing websocket. Expect to receive 'NoneType' obj has no attrib 'connected'.")
            websocket_obj.close()
