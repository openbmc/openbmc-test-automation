...
my_source [list print.tcl opt.tcl valid.tcl call_stack.tcl tools.exp cmd.tcl]
...
longoptions openbmc_host: openbmc_username:=root openbmc_password:=0penBmc\
  ...
  ftp_username:=0 ftp_password:=0 os_hostname:=0 domain:=0 gateway:=0 netmask:=0\
  mac_address:=0 dns:=0 os_repo_url:=0 test_mode:=0 quiet:=0 debug:=0
pos_parms

set valid_proc_name [list os_login boot_to_petitboot go_to_petitboot_shell install_os\
...
proc validate_parms {} {
  ...
  if {$proc_name == "install_os"} {
    valid_value ftp_username 0
    valid_password ftp_password 0
    valid_value os_hostname 0
    valid_value domain 0
    valid_value gateway 0
    valid_value netmask 0
    valid_value mac_address 0
    valid_value dns 0
    valid_value os_repo_url 0
  }
...
proc install_os {} {
  ...
  global os_host os_hostname domain gateway netmask mac_address dns
  ...
  send_wrap "b"
  set expect_result [expect_wrap [list "Installation complete* Press \
  return to quit"] "os installation complete message" 2000]
