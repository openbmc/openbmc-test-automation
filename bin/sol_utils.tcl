...
proc validate_parms {} {
  ...
  if { [lsearch -exact [list "install_os" "enable_pegas_repositories"] \
        $proc_names] != -1 } {
...
proc enable_pegas_repositories {} {

  # Enable Pegas Repositories.
  ...
  set yum_settings "\[pegas-ga-ftp3-server\]\nname=pegas-ga-ftp3-server\
    \nbaseurl=ftp://$ftp_username:$ftp_password@$os_repo_url\
    \nenabled=1\ngpgcheck=0"
  send_wrap \
    "printf \"$yum_settings\" > /etc/yum.repos.d/pegas_ga.repo"
  shell_command "yum update" $os_prompt_regex
}
