...
parser = argparse.ArgumentParser(
    ...
    description="%(prog)s will subscribe and receive event notifications when "
                + "properties change for given dbus path.",
...
parser.add_argument(
    '--dbus_path',
    default='',
    help='The path to be monitored (e.g. {"paths": ["/xyz/openbmc_project/sensors"]}).')
parser.add_argument(
    '--enable_trace',
    choices=[0, 1],
    default=0,
    help='Indicates whether trace needs to be enabled.')

args = vars(parser.parse_args())
username = args['username']
password = args['password']
dbus_path = args['dbus_path']
trace = args['enable_trace']
host = args['host']
...
def main():
    ...
    output = my_event.subscribe(dbus_path, trace)
