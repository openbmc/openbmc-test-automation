...
class FFDCCollector:
    ...
    def verify_script_env(self):
        ...
        print("\t{:<10}  {:>10}".format('ipmitool', ipmitool_version))
    ...
    def protocol_ipmi(self,
        ...
        for index, each_cmd in enumerate(list_of_cmd, start=0):
            redfish_parm = '-U ' + self.username + ' -P ' + self.password + ' -H ' \
                           + self.hostname + ' ' + each_cmd

            result = self.run_ipmitool(redfish_parm)
            if result:
                try:
                    targ_file = ffdc_actions[machine_type]['FILES'][index]
                except IndexError:
    ...
    def verify_ipmi(self):
        ...
        return(self.run_ipmitool(redfish_parm, True))
