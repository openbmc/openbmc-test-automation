...
class FFDCCollector:
    ...
    def collect_ffdc(self):
        ...
        for target_type in self.ffdc_actions.keys():
            if self.target_type != target_type:
                continue

            for k, v in self.ffdc_actions[target_type].items():
                if self.ffdc_actions[target_type][k]['PROTOCOL'][0] not in check_protocol_list:
                    check_protocol_list.append(self.ffdc_actions[target_type][k]['PROTOCOL'][0])
    ...
    def generate_ffdc(self, working_protocol_list):
        ...
        for target_type in self.ffdc_actions.keys():
            ...
            for k, v in self.ffdc_actions[target_type].items():
                ...
                protocol = self.ffdc_actions[target_type][k]['PROTOCOL'][0]
