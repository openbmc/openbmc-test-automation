...
${bmc_image_file_path}       /usr/local/share/pnor
${DECODE_DIR_PATH}           ${EMPTY}
...
Violate Secure Boot Via Corrupt Key During Host Boot
    [Documentation]  Violate secure boot via corrupt key during host boot.
    [Tags]  Violate_Secure_Boot_Via_Corrupt_Key_During_Host_Boot

    Violate Secure Boot Via Corrupt Key
    ...  SBE  ${pnor_corruption_src}  ${bmc_image_file_path}
...
Violate Secure Boot Via Corrupt Key
    ...
    ...  with corrupted key.
    [Arguments]  ${partition}  ${error_src}  ${bmc_image_file_path}
    ...
    #                      (e.g. "BC8A1E07").
    # bmc_image_file_path  BMC image path.
    ...
    scp.Put File
    ...  ${EXEC_DIR}/data/pnor_test_data/${partition}  ${bmc_image_file_path}
    ...
    BMC Execute Command  rm -rf ${bmc_image_file_path}/*
...
Collect Error Logs and Verify SRC
    ...
    Convert eSEL To Elog Format  ${DECODE_DIR_PATH}

    ${cmd}=  Catenate
    ...  grep -i ${system_reference_code} ${DECODE_DIR_PATH}/esel.out.txt
...
Suite Setup Execution
    ...
    Run Command  export PATH=$PATH:${DECODE_DIR_PATH}
    Set Environment Variable  ${DECODE_DIR_PATH}
    BMC Execute Command  rm -rf ${bmc_image_file_path}/*
