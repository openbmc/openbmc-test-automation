...
Verify Session Timeout Validation for WebUI Session
    ...
    [Tags]    Verify_Session_Timeout_Validation_SessionType_WebUI
    [Setup]    Run Keywords
    ...    Redfish.Login    AND
    ...     Delete All Redfish Sessions    AND
    ...     Close Browser  AND
    ...    Get Default Session Timeout Via Redfish  
    [Teardown]  Set Default Session Timeout Via Redfish  AND
    ...
    FOR  ${webui_timeout}  IN  @{webui_sessionTimeout_value}
        ...
        # GET the session count before login, should be 1 (default session created by Redfish).
        Get Session Member And Verify Session Count  valid_status_code=${HTTP_OK}  expected_count=${1}

        # Launch browser and login GUI, which will create a WebUI session.
        Launch Browser And Login GUI
        Navigate To Required Sub Menu  ${xpath_secuity_and_accesss_menu}  ${xpath_sessions_sub_menu}  sessions

        # GET the session count after login, should be 2 (default session created by Redfish and WebUI session).
        Get Session Member And Verify Session Count  valid_status_code=${HTTP_OK}  expected_count=${2}

        # Verify WebUI session expired after timeout.
        Sleep  ${webui_timeout+5}s
        Redfish.Login

        # Check WebUI session expired after timeout, should return unauthorized error.
        Check Session Expired After Timeout  session_instance=${session_member}

        # Verify session expired after timeout.
        ${session_error}=  Get Specific Sessions Member Instance Via Redfish  instance=${session_member}
        ...    status_codes=${HTTP_NOT_FOUND}

        # GET the session count after session expired, should be 1 (default session created by Redfish).
        Get Session Member And Verify Session Count  valid_status_code=${HTTP_OK}  expected_count=${1}
...
Get Session Member And Verify Session Count
    ...
    ${session_resp}=  Redfish.Get Properties  ${REDFISH_SESSION}  valid_status_codes=[${valid_status_code}]
...
Get Specific Sessions Member Instance Via Redfish
    ...
    # Valid_status_codes    The valid status codes for the session member instance response.
