...
Verify Display Of Role Groups Section Elements
    [Documentation]    Verify that all Role Groups section elements are displayed correctly.
    ...                This test validates:
    ...                - "Role Groups" section title is displayed.
    ...                - "Group name" and "Group privilege" column headers are displayed.
    ...                - "Select-all" checkbox is present in the table header.
    ...                - "Add role group" button is displayed.
    ...                - "Remove role group" button with trash icon is displayed.
    ...
    # Verify delete icon is present for the role group (trash icon).
    Page Should Contain Element  ${xpath_delete_group_button}
    ...  msg=Delete icon is not present for the role group.


Verify Only Two Privileges Visible In Group Privilege Options
    [Documentation]    Verify that only two privilege options are visible in the group privilege
    ...                dropdown when adding a role group. This test validates the available privilege
    ...                options match the expected LDAP configuration.
    ...                Expected options: Administrator and ReadOnly.
