...
Verify Delete Button Popup State Based On Role Group Checkbox Selection
    [Documentation]  Verify that the delete selected button's state (enabled/disabled) correctly
    ...  reflects the checkbox selection state in the LDAP role groups table.
...
Verify Delete Button State With Checkbox Selection
    ...
    ELSE
        ...
        IF  ${button_visible}
            ...
            ...  msg=Delete button should be disabled when no checkbox is selected
        ELSE
            # Button not visible is acceptable when no checkbox is selected
            Pass Execution  Delete button is not visible when no checkbox is selected (expected behavior)
        END
    END