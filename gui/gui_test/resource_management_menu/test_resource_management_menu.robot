...
Verify Able To Set Power Cap Value
    ...
    ${value}=  Evaluate  random.randint(1, 10000)  modules=random
    Input Text  ${xpath_power_cap_value}  ${value}
    Page Should Contain Element  ${xpath_asset_tag_save_button}
    Click Element  ${xpath_asset_tag_save_button}
    # Need to add verification step to check if the power cap value is set successfully. 
    # once the open issue is resolved, add verification step here.
