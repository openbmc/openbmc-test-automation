...
# Password used by KVM Roll Base Access Control test users.
...
Verify KVM Availability
    [Documentation]  Verify the KVM option is visible in Web UI and loads successfully
    [Tags]  Verify_KVM_Availability_In_Web_UI
...
Verify KVM Role Based Access Control
    ...
    [Setup]  Test Setup Execution
    [Teardown]  KVM Role Base Access Control Test Teardown  ${operator_user}  ${readonly_user}
...
Verify KVM Multiple Sessions
    ...
    # Open sessions 2-4 (${session_num} 1-3 → Connected) and session 5 (${session_num} 4 → Disconnected/rejected).
...
Verify KVM Backend Service Unavailable
    ...
    [Setup]  Test Setup Execution
    [Teardown]  Run Keywords  Close Browser  AND  BMC Execute Command  systemctl start obmc-ikvm.service
    ...
    Reload Page
    Sleep  5s
    # Verify KVM shows Disconnected status (user friendly error message)
...
KVM Role Base Access Control Test Teardown
...
Create KVM Session
    ...
    ...  behavior. Close the secondary browser session after validation.
...
Test Setup Execution
    [Documentation]  Launch browser, Login and Navigate to kvm page.
