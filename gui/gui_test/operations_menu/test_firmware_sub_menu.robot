...
Verify Existence Of All Buttons In Firmware Page At Host Power Off
    ...
    [Setup]  Run Keywords  Power Off Server  AND  Navigate To Required Sub Menu  ${xpath_operations_menu}  
...
Verify Switch To Running Image With Switch Image Button At Power-Off State
    ...
    [Teardown]  Run Keywords  Click Element  ${xpath_switch_to_running}  AND  Perform Firmware Switch And verify
...
Verify Switch To Running Image With Switch Image Button At Power-Off State With Read-Only User
    [Documentation]  Verify that a read-only user is restricted from switching to a running image when
    ...              the system is in power-off state by clicking the "Switch Image" button
...
Verify Switch To Running Image With Cancel Button At Power-Off State With Read-Only User
    [Documentation]  Verify that clicking the “Cancel” button minimizes the dialog and prevents switching to a
    ...              running image when a read-only user attempts the action in a powered‑off state.
...
Verify Existence Of All Sub Sections Under BMC And Server Section At Power On State
    ...
    [Setup]  Run Keywords  Power On Server  AND  Navigate To Required Sub Menu  ${xpath_operations_menu}
    ...      ${xpath_firmware_update_sub_menu}  firmware
