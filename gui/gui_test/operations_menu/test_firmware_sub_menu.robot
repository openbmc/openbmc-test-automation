...
...  HOST_IMAGE_FILE_PATH    Path to the BIOS single image/Multi/Partial package file.
...
Verify BMC Firmware Update
    ...
    ${bmc_version}=  Redfish.Get Attribute  /redfish/v1/Managers/${MANAGER_ID}  FirmwareVersion
    Should Be Equal  ${bmc_version}  ${image_version}

    # Verify the version displayed on Web UI matches.
    Wait Until Keyword Succeeds  1min  1s  Page Should Contain  ${image_version}


Verify Valid Signed BMC Firmware
    [Documentation]  Verify signature for BMC firmware.
    [Tags]  Verify_Valid_Signed_BMC_Firmware
    [Teardown]  Remove Directory  /tmp/fw_verify  recursive=True

    Create Directory  /tmp/fw_verify
    ${rc}=  Run And Return Rc  tar xf ${IMAGE_FILE_PATH} -C /tmp/fw_verify/
    Should Be Equal As Integers  ${rc}  0
    ...  msg=Failed to extract firmware tar: ${IMAGE_FILE_PATH}

    # Verify MANIFEST signature:
    # openssl dgst -sha256 -verify publickey -signature MANIFEST.sig MANIFEST
    ${manifest_verify}=  Run
    ...  openssl dgst -sha256 -verify /tmp/fw_verify/publickey -signature /tmp/fw_verify/MANIFEST.sig /tmp/fw_verify/MANIFEST
    Should Contain  ${manifest_verify}  Verified OK
    ...  msg=MANIFEST signature verification failed: ${manifest_verify}
    Log To Console  MANIFEST: ${manifest_verify}

    # Verify image-kernel signature:
    # openssl dgst -sha256 -verify publickey -signature image-kernel.sig image-kernel
    ${kernel_verify}=  Run
    ...  openssl dgst -sha256 -verify /tmp/fw_verify/publickey -signature /tmp/fw_verify/image-kernel.sig /tmp/fw_verify/image-kernel
    Should Contain  ${kernel_verify}  Verified OK
    ...  msg=image-kernel signature verification failed: ${kernel_verify}
    Log To Console  image-kernel: ${kernel_verify}

    # Verify image-rofs signature:
    # openssl dgst -sha256 -verify publickey -signature image-rofs.sig image-rofs
    ${rofs_verify}=  Run
    ...  openssl dgst -sha256 -verify /tmp/fw_verify/publickey -signature /tmp/fw_verify/image-rofs.sig /tmp/fw_verify/image-rofs
    Should Contain  ${rofs_verify}  Verified OK
    ...  msg=image-rofs signature verification failed: ${rofs_verify}
    Log To Console  image-rofs: ${rofs_verify}

    # Verify image-u-boot signature:
    # openssl dgst -sha256 -verify publickey -signature image-u-boot.sig image-u-boot
    ${uboot_verify}=  Run
    ...  openssl dgst -sha256 -verify /tmp/fw_verify/publickey -signature /tmp/fw_verify/image-u-boot.sig /tmp/fw_verify/image-u-boot
    Should Contain  ${uboot_verify}  Verified OK
    ...  msg=image-u-boot signature verification failed: ${uboot_verify}
    Log To Console  image-u-boot: ${uboot_verify}

    # Verify image-rwfs signature:
    # openssl dgst -sha256 -verify publickey -signature image-rwfs.sig image-rwfs
    ${rwfs_verify}=  Run
    ...  openssl dgst -sha256 -verify /tmp/fw_verify/publickey -signature /tmp/fw_verify/image-rwfs.sig /tmp/fw_verify/image-rwfs
    Should Contain  ${rwfs_verify}  Verified OK
    ...  msg=image-rwfs signature verification failed: ${rwfs_verify}
    Log To Console  image-rwfs: ${rwfs_verify}
...
Verify BMC Running Firmware Version
    ...
    # Verify the version displayed on Web UI matches.
    Page Should Contain  ${bmc_version}
...
Verify BMC Backup Firmware Version
    ...
    Redfish.Login
    ${backup_version}=  Get Backup Firmware Version

    # Verify the backup version is displayed on Web UI.
    Page Should Contain  ${backup_version}
...
Verify Host Running Firmware Version
    ...
    # Verify the version displayed on Web UI matches.
    Page Should Contain  ${host_version}
...
Get Backup Firmware Version
    [Documentation]  Get the backup (non-functional) BMC firmware version via Redfish.
    ...
    FOR  ${member_uri}  IN  @{members}
        ...
        ${version}=  Set Variable  ${inv_data.dict.get('Version', '')}
        ${name}=  Set Variable  ${inv_data.dict.get('Name', '')}
...
Perform Firmware Update
    ...
    # Wait for host to come back up (poll every 30 seconds, up to 15 minutes).
    # Wait Until Keyword Succeeds  15 min  30 sec  Redfish.Login
