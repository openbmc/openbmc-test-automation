...
Verify Server Power Cap Setting Is On
    ...
    Run Keyword If  '${checkbox_initial_state}' == 'False'  Click Element At Coordinates  ${xpath_power_ops_checkbox}  0  0
...
Save Initial Checkbox State
    [Documentation]  Save the initial state of the check box.
    ...
    Set test variable    ${checkbox_initial_state}    ${resp}


Restore Initial Checkbox State
    [Documentation]  Restore the initial state of the check box.

    ${resp}=  Run Keyword And Return Status  Checkbox Should Be Selected  ${xpath_power_ops_checkbox}
    Run Keyword If  ${resp} != ${checkbox_initial_state}  Click Element At Coordinates  ${xpath_power_ops_checkbox}  0  0
