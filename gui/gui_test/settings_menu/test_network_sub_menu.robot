...
${xpath_add_static_ipv4_addr_btn_eth0}   (//*[@data-test-id="add-static-ipv4"])[1]
${xpath_add_static_ipv4_addr_btn_eth1}   (//*[@data-test-id="add-static-ipv4"])[2]
${xpath_add_static_ipv6_addr_btn_eth0}   (//*[@data-test-id="add-static-ipv6"])[1]
${xpath_add_static_ipv6_addr_btn_eth1}   (//*[@data-test-id="add-static-ipv6"])[2]
...
${xpath_eth0_autoconfig_button}          (//dt[normalize-space()='IPv6 auto config']/following-sibling::dd[1]//label[@class='form-check-label'])[1]
${xpath_eth1_autoconfig_button}          (//dt[normalize-space()='IPv6 auto config']/following-sibling::dd[1]//label[@class='form-check-label'])[2]
${xpath_eth0_dhcpv6_button}              (//dt[normalize-space()='DHCP']/following-sibling::dd[1]//label[@class='form-check-label'])[2]
${xpath_eth1_dhcpv6_button}              (//dt[normalize-space()='DHCP']/following-sibling::dd[1]//label[@class='form-check-label'])[4]
...
Verify IPv4 Configuration Remains Intact When SLAAC Is Enabled
    [Documentation]  Enable SLAAC and verify there is no change in IPv4 configuration.
    [Tags]  Verify_IPv4_Configuration_Remains_Intact_When_SLAAC_Is_Enabled
...
Configure Staticv6 On Eth0 And DHCPv6 On Eth1 With Eth0 In Static And Eth1 In DHCPv4
    [Documentation]  Set eth0 static and eth1 DHCPv4, Verify staticv6 on eth0 and DHCPv6 on eth1.
    [Tags]  Configure_Staticv6_On_Eth0_And_DHCPv6_On_Eth1_With_Eth0_In_Static_And_Eth1_In_DHCPv4
...
Configure Staticv6 And DHCPv6 On Eth1 With Eth0 And Eth1 In Staticv4
    [Documentation]  Set eth0 and eth1 to staticv4, verify staticv6 and DHCPv6 on eth1.
    [Tags]  Configure_Staticv6_And_DHCPv6_On_Eth1_With_Eth0_And_Eth1_In_Staticv4
...
Configure Eth0 Eth1 In Staticv4 And Verify Configuring Staticv6 DHCPv6 On Eth0
    [Documentation]  Set up both eth0 and eth1 in Static IPv4 mode, add a Static IPv6 address on eth0,
    ...    and enable DHCPv6 on eth0.
    ...
    ...  AND  Run Keyword And Ignore Error  Delete IP Address And Verify  ipv4  ${test_ipv4_addr}
...
Configure Staticv6 On Eth1 Enable DHCPv4 Verify Staticv6 Persists
    [Documentation]  Configure static IPv6 on eth1, enable DHCPv4 on eth1, and verify
    ...  the IPv6 configuration remains unchanged.
    [Tags]  Configure_Staticv6_On_Eth1_Enable_DHCPv4_Verify_Staticv6_Persists
    [Setup]  Add Static IPv6 Address And Verify Via GUI  ${test_ipv6_addr_2}
    ...      ${test_prefix_length}  Success  None  2
    [Teardown]  Run Keywords
    ...  Run Keyword And Ignore Error  Delete IP Address And Verify  ipv6  ${test_ipv6_addr_2}  2
...
Cancel And Verify Network Heading
    ...
    Reload Page
    Wait Until Element Is Not Visible   ${xpath_page_loading_progress_bar}  timeout=120s
    Wait Until Element Contains  ${xpath_network_heading}  Network
...
Toggle SSH And Verify Via IP Protocol
    ...
    FOR  ${channel}  IN  1  2
        IF  '${ip_protocol}' == 'ipv4' or '${ip_protocol}' == 'both'
            Verify SSH For Protocol  ipv4  ${channel}  ${ssh_action}  ${ipv4_link_local_prefix}
        END

        IF  '${ip_protocol}' == 'ipv6' or '${ip_protocol}' == 'both'
            Verify SSH For Protocol  ipv6  ${channel}  ${ssh_action}  ${link_local_addr}
        END
    END


Verify SSH For Protocol
    [Documentation]  Verify SSH connectivity for a specific IP protocol on a channel.
    [Arguments]  ${protocol}  ${channel}  ${ssh_action}  ${link_local_prefix}

    # Description of argument(s):
    # protocol            IP protocol: 'ipv4' or 'ipv6'.
    # channel             Ethernet channel number: 1 (eth0) or 2 (eth1).
    # ssh_action          Expected SSH state: 'enable' or 'disable'.
    # link_local_prefix   Link-local prefix to skip (e.g., '169.254' or 'fe80').

    ${ip_list}=  Collect All IP Addresses On Both Interfaces  ${protocol}  ${channel}
    ${list_length}=  Get Length  ${ip_list}
    IF  ${list_length} > 0
        Verify SSH Connectivity  ${ip_list}  ${ssh_action}  ${link_local_prefix}
    ELSE
        Log  No ${protocol} addresses found on channel ${channel}  level=WARN
    END
    ...
    FOR  ${ip_addr}  IN  @{ip_list}
        ...
        ELSE
            TRY
                ...
                ...  msg=FAIL: SSH login unexpectedly succeeded via ${ip_addr} after disabling SSH. Expected connection to be refused.
            FINALLY
...
Verify Only One IPv4 Origin Type Is Present
    ...
    ${ipv4_elems}=  Get WebElements  ${ipv4_addr_origin_elements}
    ${elem_count}=  Get Length  ${ipv4_elems}
    Should Be True  ${elem_count} > 0
    ...  msg=No IPv4 addresses found in grid. At least one IPv4 address must be configured to verify origin uniqueness.
...
Toggle DHCP Property And Verify Gateway Intact
    ...
    Wait Until Element Is Visible  ${xpath_eth0_ipv4_default_gateway}  timeout=30s
    ...
    # Note: Toggle DHCPv4 Property And Verify already verifies the toggle succeeded
    # by comparing Redfish and GUI values before and after the toggle operation.
...
Configure Eth0 Static Eth1 DHCPv4 Teardown
    ...
    # ${eth1_interface} set with scope=TEST by Enable DHCPv4 On Eth1 Via Redfish And Refresh GUI in setup.
