...
Get CPU Min Frequency Limit
    [Documentation]  Get CPU minimum assignable requency.

    # lscpu | grep min  returns
    # CPU min MHz:           1983.0000

    ${cmd}=  Catenate  lscpu | grep min  | tr -dc '0-9.\n'
    ${cpu_info}  ${stderr}  ${rc}=  OS Execute Command  ${cmd}
    [Return]  ${cpu_info}


Get CPU Min Frequency
    [Documentation]  Get CPU current assigned requency.
...
Get CPU Max Frequency Limit
    [Documentation]  Get CPU maximum assignable requency.
