...
Execute Command On OS
    ...
    Should Be Empty  ${stderr}  msg=Faied running ${command}, stderr=${stderr}
    [Return]  ${stdout}
...
Is HTX Running
    ...
    ...  Return "True" if HTX is running, "False" otherwise.

    ${status}=  Execute Command On OS  htxcmdline -status

    ${match_count_idle}=  Count Values In List  ${status}
    ...  Daemon state is <IDLE>
    ${match_count_notrun}=  Count Values In List  ${status}
    ...  No MDT is currently running
    ${running}=  Evaluate  not (${match_count_idle} or ${match_count_notrun})
    [Return]  ${running}
...
Create JSON Inventory File
    ...
    ...  msg=File does not exist ${json_file_path}
