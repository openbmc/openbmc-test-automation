...
Get GPU Power
    [Documentation]  Report the GPU power dissipation.
...
Get GPU Temperature
    [Documentation]  Get the highest NVIDIA GPU temperature.
...
Get GPU Clock Limit
    ...
    # nvidia-smi --query-gpu=clocks.max.gr --format=csv  retuens
    # 1530 MHz
    # 1530 MHz
    # 1530 MHz
    # 1530 MHz
...
Get GPU Clock
    ...
    # nvidia-smi --query-gpu=clocks.gr --format=csv  retuens
    # 1230 MHz
    # 1230 MHz
    # 135 MHz
    # 150 MHz
...
Count GPUs From BMC
    ...
    \  ${num_bmc_gpus}=  Run Keyword If  ${present}  Run Keyword If  ${state}
