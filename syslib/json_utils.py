...
J_FILES_DO_NOT_MATCH = 2
J_INPUT_FILE_DOES_NOT_EXIST = 3
J_IO_EXCEPTION_READING_FILE = 4
J_INPUT_FILE_MALFORMED = 5
    ...
    if (os.path.exists(initial_json_file)
        ...
        with open(initial_json_file, 'r') as f:
            try:
                ...
                f.close()
                return J_IO_EXCEPTION_READING_FILE
            except ValueError:
                f.close()
                return J_INPUT_FILE_MALFORMED
            else:
                f.close()
        ...
        if ((len(initial) > 0) and (json_reorder(
            ...
            return J_FILES_MATCH
        else:
            return J_FILES_DO_NOT_MATCH
    else:
        # os.path does not exist for one or both input files
        return J_INPUT_FILE_DOES_NOT_EXIST
###############################################################################
