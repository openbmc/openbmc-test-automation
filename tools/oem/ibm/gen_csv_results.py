...
def parse_output_xml(i_source, i_dest):
    ...
    l_system_info = get_system_details(i_source)
    ...
    for testcase in collectDataObj.testData:
        # Test category : Suite name (function area) and test case name
...
def format_datetime(xml_datetime):
    r"""
...
def get_system_details(source):
    ...
    for node in tree.iter('msg'):
		...
		# Example: ${output}= Witherspoon BMC logged in the XML as msg
        # TODO: Update the meta data with new generic string
		if '${output} = Witherspoon BMC' in node.text:
