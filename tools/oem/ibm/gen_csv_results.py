...
def parse_output_xml(i_source, i_dest):
    ...
    l_env = 'HW'
    l_proc = 'P9'
	# TODO - Both variable needs to be populated
	# Working on it.. please hold on review
...
def format_datetime(datetime):
    ...
    l_str = datetime.strptime(datetime, "%Y%m%d %H:%M:%S.%f")
    # 2017-02-06-05-05-19
    l_str = l_str.strftime("%Y-%m-%d-%H-%M-%S")
    return str(l_str)


def get_bmc_version(source):
    r"""
    Get the BMC version data from output.xml generated by robot and return it.
    and return it.

    Description of arguments:
        source  The output.xml absolute file path.

    The data returned will be in the following format: v1.99.1-96-g2a46570
    """
    bmc_version = ""
