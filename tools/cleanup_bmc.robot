...
@{skip_list}=  /run/initramfs/rw/cow/etc/*  /run/initramfs/rw/cow/usr/lib/python2.7/*
...
Remove Files
    ...
    ${file_count1}=  Execute Command  find ${CLEANUP_DIR_PATH} | wc -l
    Write  cd ${CLEANUP_DIR_PATH}
    Write  find $PWD \\( ${skip_list_string} \\) | xargs rm
    Write  find $PWD \\( ${skip_list_string} \\) | xargs rmdir
