...
@{vpd_component_read}          PN  SN  Locationcode
@{vpd_fields}                  PN  SN
...
Verify BMC Manager VPD Data Via Vpdtool
    [Documentation]  Verify the BMC manager VPD details via vpdtool output.
    [Tags]  Verify_BMC_Manager_VPD_Data_Via_Vpdtool
    [Template]  Verify VPD Data Via Vpdtool

    # Component     Field
    BMC Manager     PN
    BMC Manager     SN
    BMC Manager     LocationCode
...
Verify VPD Field Read Operation
    ...
    FOR  ${field}  IN  @{vpd_fields}
         ${match_key_exists}=  Run Keyword And Return Status  Dictionary Should Contain Key  ${vpdtool_res}  ${field}
...
Verify VPD Data Via Vpdtool
    ...
    ...  ELSE IF  '${component}' == 'BMC Manager'  Set Variable  /system/chassis/motherboard/bmc 
    ...
    ...  '${field}' == 'SerialNumber' or '${field}' == 'SN'  xyz.openbmc_project.Inventory.Decorator.Asset SerialNumber
    ...  '${field}' == 'PN'  xyz.openbmc_project.Inventory.Decorator.Asset PartNumber

    ${cmd}=  Catenate  ${CMD_GET_PROPERTY_INVENTORY} /xyz/openbmc_project/inventory${component_url}
    ...  ${busctl_field}
    ${cmd_output}=  BMC Execute Command  ${cmd}
    ...
    Should Be Equal As Strings  ${vpd_records["${component_url}"]["${field}"]}
    ...  ${cmd_output[0].split('"')[1].strip('"')}
