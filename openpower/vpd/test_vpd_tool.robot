...
Verify VPD Component Read Operation
    ...
    FOR  ${vpd_field}  IN  @{fields}
        ${match_key_exists}=  Run Keyword And Return Status  Dictionary Should Contain Key  ${vpdtool_res}  ${vpd_field}
          IF  '${match_key_exists}' == 'True'
              ...
              Log  vpd_records['${component}']['${vpd_field}']
              Log  ['${cmd_output[0].split('"')[1].strip('"')}']
              Valid Value  vpd_records['${component}']['${vpd_field}']  ['${cmd_output[0].split('"')[1].strip('"')}']
          ELSE
             Continue For Loop
          END
    END
