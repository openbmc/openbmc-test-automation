...
Verify Small Partition File Upload And Delete
    ...
    # file_name  size_kb  part_name  expect_resp_code     expected_msg      delete_part  user_name
...
Verify Multiple Files Upload
    ...
    15KB_file    15       15KB       ${HTTP_OK}           ${UPLOADED_MSG}   ${False}     ${OPENBMC_USERNAME}
...
Verify Non-Admin User Is Forbidden To Upload Partition File
    ...
    # file_name  size_kb  part_name  expect_resp_code     expected_msg      delete_part  user_name
    10KB_file    10       10KB       ${HTTP_FORBIDDEN}    ${FORBIDDEN_MSG}  ${False}     operator_user
...
Verify Delete Partition When Partition Does Not Exist
    ...
    # partition_name    expect_resp_code      user_name
...
Delete All Sessions And Login Using Given User
    ...
    [Arguments]   ${user_name}=${OPENBMC_USERNAME}

    # Description of argument(s):
    # user_name            Username to login. Default is OPENBMC_USERNAME.
    #                     Ex: root, operator_user, admin_user, readonly_user etc.

    Delete All Sessions
    ${password}=  Set Variable If  '${user_name}' == '${OPENBMC_USERNAME}'  ${OPENBMC_PASSWORD}  TestPwd123
    Initialize OpenBMC  rest_username=${user_name}  rest_password=${password}


Upload File To Create Partition Then Delete Partition
    [Documentation]  Upload file to create partition the delete partition.
    [Arguments]  ${file_name}=dummyfile  ${size_kb}=15  ${part_name}=p1  ${expect_resp_code}=${HTTP_OK}
    ...  ${expected_msg}=File Created  ${delete_part}=${True}  ${user_name}=${OPENBMC_USERNAME}

    # Description of argument(s):
    # file_name           Name of the test file to be created.
    # part_name           Name of the partition on BMC.
    ...
    # delete_part         Partition will be deleted if this is True.
    # user_name            Login user_name

    # Create a session with given user to test upload partition file.
    Run Keyword If  '${user_name}' != '${OPENBMC_USERNAME}'
    ...  Delete All Sessions And Login Using Given User  ${user_name}
    ...
    ${resp}=  Put Request  openbmc  /ibm/v1/Host/ConfigFiles/${part_name}  &{data}
    ...
    ${description}=  Set Variable If  ${expect_resp_code} == ${HTTP_FORBIDDEN}
    ...  ${FORBIDDEN_MSG}  ${description}
    ...
    Verify Partition Available On BMC  ${part_name}  ${upload_success}  ${user_name}

    # Delete partition and verify on BMC.
    Run Keyword If  ${delete_part} == ${True}  Delete Partition And Verify On BMC
    ...  ${part_name}  user_name=${user_name}
...
Delete Partition And Verify On BMC
    ...
    [Arguments]  ${partition_name}  ${expect_resp_code}=${HTTP_OK}  ${user_name}=${OPENBMC_USERNAME}
    ...
    # user_name           Username to login, if other than OPENBMC_USERNAME user.
    ...
    Run Keyword If  '${user_name}' != '${OPENBMC_USERNAME}'  Run Keywords
    ...  Delete All Sessions And Login Using Given User  ${user_name}  AND
    ...  Set Variable  ${expect_resp_code}  ${HTTP_FORBIDDEN}

    Delete Partition  ${partition_name}  ${expect_resp_code}
    Verify Partition Available On BMC  ${partition_name}  ${False}  ${user_name}
...
Verify Partition Available On BMC
    ...
    [Arguments]  ${partition_name}=${EMPTY}  ${operation_status}=${True}  ${user_name}=${OPENBMC_USERNAME}
    ...
    # user_name          Username used to upload/delete. Default is ${OPENBMC_USERNAME}.
