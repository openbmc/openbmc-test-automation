...
Verify Small Partition File Upload And Delete
    ...
    # file_name  size_kb  partition_name  expect_resp_code     msg                      delete_partition
    201KB_file   201      201KB           ${HTTP_BAD_REQUEST}  ${MAX_SIZE_UPLOAD_MSG}
    15KB_file    15       15KB            ${HTTP_OK}           ${FILE_UPLOADED_MSG}
    200KB_file   200      200KB           ${HTTP_OK}           ${FILE_UPLOADED_MSG}
...
Verify Multiple Files Upload
    ...
    100KB_file   100      100KB           ${HTTP_OK}           ${FILE_UPLOADED_MSG}     ${False}
...
Verify Read Partition
    ...
    Read Partition And Verify Content  ${partition_name}  Sample Content to test partition file upload
...
Upload File To Create Partition Then Delete Partition
    ...
    Verify Partition Availability  ${partition_name}  ${upload_success}
    ...
    Verify Partition Availability  ${partition_name}  ${False}


Verify Partition Availability
    [Documentation]  Verify partition availability.
    [Arguments]  ${partition_name}=p1  ${operation_status}=${True}
    ...
    Should Be Equal  ${partition_found}  ${operation_status}
