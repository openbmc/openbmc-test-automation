...
Create A Session With Client-id And Verify
   [Documentation]  Create a session with client id and verify client id is same.
   [Tags]  Create_A_Session_With_Client-id_And_Verify
   [Template]  Create A Session With Client-id
...
Create A Session With Client-id
    ...
    Verify A Session Created With Client-id  ${client_id}  ${resp['Id']}


Verify A Session Created With Client-id
