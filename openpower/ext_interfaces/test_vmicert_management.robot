...
Get Concurrent Corrupted CSR Requests From Multiple Admin Users
    ...
    FOR    ${i}    IN RANGE    ${5}
        ${dict}=    Execute Process Multi Keyword
        ...    ${5}
        ...    Get Certificate Signed By VMI ${OPENBMC_USERNAME} ${OPENBMC_PASSWORD} ${True} ${False} ${HTTP_BAD_REQUEST}
        ...    Get Certificate Signed By VMI ${OPENBMC_USERNAME} ${OPENBMC_PASSWORD} ${True} ${False} ${HTTP_BAD_REQUEST}
        ...    Get Certificate Signed By VMI ${OPENBMC_USERNAME} ${OPENBMC_PASSWORD} ${True} ${False} ${HTTP_BAD_REQUEST}
...
Get Subject
    ...
    IF    ${is_csr_file}
        ${subject}=    Run    openssl req -in ${file_name} -text -noout | grep Subject:
    ELSE
        ${subject}=    Run    openssl x509 -in ${file_name} -text -noout | grep Subject:
    END
...
Get Public Key
    ...
    IF    ${is_csr_file}
        ${PublicKey}=    Run    openssl req -in ${file_name} -noout -pubkey
    ELSE
        ${PublicKey}=    Run    openssl x509 -in ${file_name} -noout -pubkey
    END
