...
Configure VMI Static IP Address On VMI Via Different User Roles And Verify
    [Documentation]  Configure vmi static IP address via different user roles And verify.
    [Tags]  Configure_VMI_Static_IP_Address Via_Different_User_Roles_And_Verify
    [Setup]  Create Users With Different Roles  users=${USERS}  force=${True}
    [Template]  Config Static IP Address Using Different Users
...
Delete VMI Static IP Address Via Different User Roles And Verify
    ...
    [Setup]  Create Users With Different Roles  users=${USERS}  force=${True}
    [Template]  Delete Static IP Address Using Different Users
    [Teardown]  Run Keywords  Redfish.Login  AND  Delete BMC Users Via Redfish  users=${USERS}
...
Update VMI Static IP Address Via Different User Roles And Verify
    ...
    # username     password     ip_address  gateway  nemask       valid_status_code
    admin_user     TestPwd123   10.5.5.1    0.0.0.0  255.255.0.0  ${HTTP_ACCEPTED}
    operator_user  TestPwd123   10.5.5.2    0.0.0.0  255.255.0.0  ${HTTP_FORBIDDEN}
    readonly_user  TestPwd123   10.5.5.3    0.0.0.0  255.255.0.0  ${HTTP_FORBIDDEN}
    noaccess_user  TestPwd123   10.5.5.4    0.0.0.0  255.255.0.0  ${HTTP_FORBIDDEN}
...
Read VMI Network Configuration Via Different User Roles And Verify
    ...
    [Setup]  Create Users With Different Roles  users=${USERS}  force=${True}
    [Template]  Read VMI NetworK Configuration Using Different Users
    [Teardown]  Run Keywords  Redfish.Login  AND  Delete BMC Users Via Redfish  users=${USERS}
...
Delete Static IP Address Using Different Users
    ...
    Run Keywords  Redfish.Logout


Config Static IP Address Using Different Users
...
Read VMI NetworK Configuration Using Different Users
