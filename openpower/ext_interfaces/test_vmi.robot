...
Suite Setup Execution
    ...
    Run Keywords  Redfish.Login  AND  Redfish Power On
    ${active_channel_config}=  Get Active Channel Config
    Set Suite Variable   ${active_channel_config}
    ${vmi_network_conf}=  Create Dictionary
    ${resp}=  Redfish.Get
    ...  /redfish/v1/Systems/hypervisor/EthernetInterfaces/${active_channel_config['${CHANNEL_NUMBER}']['name']}
    ${ip_resp}=  Evaluate  json.loads('''${resp.text}''')  json
    ${length}=  Get Length  ${ip_resp["IPv4StaticAddresses"]}
    Run Keyword If  ${length} != ${0}
    ...  Set To Dictionary  ${vmi_network_conf}  IPv4_Address=${ip_resp["IPv4Addresses"][0]["Address"]}
    ...  IPv4_Gateway=${ip_resp["IPv4Addresses"][0]["Gateway"]}
    ...  IPv4_Netmask=${ip_resp["IPv4Addresses"][0]["SubnetMask"]}
    Set Global Variable  ${vmi_network_conf}
