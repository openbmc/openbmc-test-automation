...
Suite Setup Execution
    ...
    Login To OS  ${OPENBMC_HOST}  ${OPENBMC_USERNAME}  ${OPENBMC_PASSWORD}    
    Check Avahi Package
    Check Avahi Service Status

Check Avahi Package
    [Documentation]  To check for avahi-tool package.
...
Discover BMC With Service Type
    ...
    ${publish_service}=  Evaluate
    ...  json.load(open('${code_base_dir_path}data/BMC_publish_service.json'))  modules=json
    Rprint Vars  publish_service
    ${resp_service}  ${stderr}=  Execute Command  ${publish_service['${service_type}']}  return_stderr=True
