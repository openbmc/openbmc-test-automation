...
Verify Valid Lock Transactions Release
    ...
    ${transaction_id1}=  Run Keyword  Acquire Lock On A Given Resource
    ...  Read  ${TWO_SEG_FLAG_2}  ${234}
    ${locks_before}=  Run Keyword  Get Locks List  ${SESSION_ID}

    ${transaction_id2}=  Run Keyword  Acquire Lock On A Given Resource
    ...  Read  ${TWO_SEG_FLAG_3}  ${234}
    ${transaction_id3}=  Run Keyword  Acquire Lock On A Given Resource
    ...  Read  ${TWO_SEG_FLAG_2}  ${234}
    ${transaction_id4}=  Run Keyword  Acquire Lock On A Given Resource
    ...  Read  ${TWO_SEG_FLAG_3}  ${234}

    ${transaction_ids}=  Create List  ${transaction_id2}  ${transaction_id3}  ${transaction_id4}
    Release Locks  ${transaction_ids}

    ${locks_after}=  Run Keyword  Get Locks List  ${SESSION_ID}
...
Verify Locks Created By One Session Cannot Be Delete By Another Session
    [Documentation]  Verify locks created by one session cannot be deleted by another session.
    [Tags]  Verify_Locks_Created_By_One_Session_Cannot_Be_Delete_By_Another_Session.
