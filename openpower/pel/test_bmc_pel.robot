...
Error Logging Rotation Policy
    ...
    IF    ${disk_usage_percentage} > ${max_allocated_space_percentage}
        ${trimmed_as_expected}=    Evaluate    ${percent_diff} <= 0.5
    ELSE
        ${trimmed_as_expected}=    Set Variable    True
    END
...
Create Error Log
    ...
    FOR    ${i}    IN RANGE    0    ${count}
        ${cmd}=    Set Variable If
        ...    '${error_severity}' == 'Informational' and '${error_creator}' == 'BMC'
        ...    ${CMD_INFORMATIONAL_ERROR}
        ...    '${error_severity}' == 'Informational' and '${error_creator}' == 'HOST'
        ...    ${CMD_INFORMATIONAL_HOST_ERROR}
        ...    '${error_severity}' == 'Predictive' and '${error_creator}' == 'BMC'
        ...    ${CMD_PREDICTIVE_ERROR}
        ...    '${error_severity}' == 'Unrecoverable' and '${error_creator}' == 'BMC'
        ...    ${CMD_UNRECOVERABLE_ERROR}
        ...    '${error_severity}' == 'Unrecoverable' and '${error_creator}' == 'HOST'
...
Get PEL Log IDs
    ...
    FOR    ${id}    IN    @{pel_ids}
        ...
        IF    '${pel_field_output}' in @{pel_field_value}
            Append To List    ${pel_id_list}    ${id}
        END
    END
