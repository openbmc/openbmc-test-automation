...
Verify Error Logging Rotation Policy
    ...
    [Template]  Test Error Logging Rotation Policy
...
Error Logging Rotation Policy
    ...
    # Create 3001 information logs. The logging disk capacity limit is set to 20MB and the log count
    # limit is 3000. Once log counnt crosses 3000, i.e. number of logs reaches 3001, information log
    # sizes come to around 3 MB, unrecoverable and predictive is around 6 MB (which is 15%, 30% and
    # 30% of total disk capacity - 3MB or 6MB usage / 20MB total log allocation ). It is found that
    # till 3000 log count, disk usage increases but once log count is 3001, disk usage becomes
    # 3.1 MB for information log, 6.1 is for unrecoverable and predictive error logs.
