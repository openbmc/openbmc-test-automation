...
Verify FRU Callout
    ...
    # TODO: Compare CCIN field of PEL callout with motherboard CCIN.
    Should Match Regexp  ${pel_callout_section['Callouts'][0]['CCIN']}  [a-zA-Z0-9]

    # Verify Part Number field of PEL callout with motherboard's Part Number.
    ${busctl_output}=  BMC Execute Command
    ...  ${CMD_INVENTORY_PREFIX} xyz.openbmc_project.Inventory.Decorator.Asset PartNumber
    Should Be Equal  ${pel_callout_section['Callouts'][0]['Part Number']}
    ...  ${busctl_output[0].split('"')[1].strip('"')}
