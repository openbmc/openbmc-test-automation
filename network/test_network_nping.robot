...
Send Network Packets
    ...
    IF  '${packet_type}' == 'icmp'
        VAR  ${cmd_buff}  echo ${CLIENT_PASSWORD} | sudo -S nping --delay ${delay} ${host} -c ${count} --${packet_type}
    ELSE
        VAR  ${cmd_buff}  echo ${CLIENT_PASSWORD} | sudo -S nping --delay ${delay} ${host} -c ${count} -p ${port} --${packet_type}
    END
