...
Suite Setup Execution
    ...
    END
    IF    ${program_cpld} == 1    Get File From SFTP Server    ${cpld_firmware1}
    IF    ${program_cpld} == 1    Get File From SFTP Server    ${cpld_firmware2}

    scp.Open connection    ${OPENBMC_HOST}    username=${OPENBMC_USERNAME}
    ...    password=${OPENBMC_PASSWORD}
    Put File To BMC    ${readid_svf}
    IF    ${program_cpld} == 1    Put File To BMC    ${readusercode_svf}
    IF    ${program_cpld} == 1    Put File To BMC    ${cpld_firmware1}
    IF    ${program_cpld} == 1    Put File To BMC    ${cpld_firmware2}
    Sleep    5s
