...
Delete Admin User And Verify SSH SOL Login Fails
    [Documentation]  Delete admin user and verify that SSH login on SOL port 2200 fails.
...
Expire Admin Password And Verify SSH SOL Login Fails
    ...
    # Verify that SSH login on SOL port with default password.
    Verify SSH Login via SOL Port  ${admin_user}  ${default_adminuser_passwd}  ${True}
...
Delete User And Close Connection And Logout
    [Documentation]  Delete the specified user account, close all connections and logout.
    [Arguments]  ${user}
