...
def set_ffdc_defaults(ffdc_dir_path=None,
    ...
    NOTE: If global variable new_ffdc_default is set to ${1}, this function
    will create default values in a newer way.  Otherwise, its behavior
    will remain unchanged.
    """

    new_ffdc_default = BuiltIn().get_variable_value("${new_ffdc_default}")
    ...
    if ffdc_dir_path is None:
        if new_ffdc_default:
            try:
                ...
                error_message = "Unable to set default value for " +\
                                "ffdc_dir_path.  Tried to use " +\
                                "FFDC_DIR_PATH which has no value."
