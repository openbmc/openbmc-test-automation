...
Initialize OpenBMC
    ...
    Rqprint Timen  Creating an http session to OBMC.
    ...
    Rqprint Timen  Logging in to OBMC via REST interface.
    ${status}  ${resp}=  Run Keyword And Ignore Error  Post Request  openbmc
    ...  /login  data=${data}  headers=${headers}

    ${error_message}=  Run Keyword And Ignore Error  Sprint Error  ${resp}
    Run Keyword If  '${status}' != 'PASS'  Rprint Error Report  ${resp}
    Should Be Equal  ${status}  PASS  msg=${error_message}
