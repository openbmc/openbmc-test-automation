...
OpenBMC Get Request
    ...
    END
    # Retry with a fresh login on two conditions:
    # 1) GET On Session raises (e.g. lost 'openbmc' alias).
    # 2) BMC returns 401 — happens when Delete All Redfish Sessions (called at
    #    the end of FFDC collection) destroys the BMC-side session without
    #    clearing ${XAUTH_TOKEN}, causing Initialize OpenBMC to skip re-login.
    ${status}  ${resp}=  Run Keyword And Ignore Error
    ...  GET On Session  openbmc  ${base_uri}  &{kwargs}  timeout=${timeout}  expected_status=any
    IF  '${status}' != 'PASS' or ${resp.status_code} == ${HTTP_UNAUTHORIZED}
