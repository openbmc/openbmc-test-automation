...
Upload And Activate Image
    ...
    Log To Console  Firmware upload started: ${image_file_path}
    ...
    Log To Console  Firmware upload completed: ${image_file_path}

    Log To Console  Firmware verify image started: ${image_version}
    ${ret}  ${version_id}=  Verify Image Upload  ${image_version}
    Should Be True  ${ret}
    Log To Console  Firmware verify image completed: ${version_id}

    # Verify the image is 'READY' to be activated or if it's already active,
    # set priority to 0 and reboot the BMC.
    Log To Console  Firmware activation state check started: ${version_id}
    ...
    IF  '${skip_if_active}' == 'true' and '${activation}' == '${ACTIVE}'
        Log To Console  Firmware activation state check completed: ${activation}
    ...
    Log To Console  Firmware activation started: ${version_id}
    ${args}=  Create Dictionary  data=${REQUESTED_ACTIVE}
    Write Attribute  ${SOFTWARE_VERSION_URI}${version_id}
    ...  RequestedActivation  data=${args}
    Log To Console  Firmware activation request completed: ${version_id}

    ${software_state}=  Read Properties  ${SOFTWARE_VERSION_URI}${version_id}  timeout=${30}
    Log To Console  Firmware requested activation state: ${software_state}[RequestedActivation]
    ...
    Log To Console  Firmware activation completed: ${version_id}
...
Attempt To Reboot BMC During Image Activation
    ...
    # the next OpenBMC Get Request call.
