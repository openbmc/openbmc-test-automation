...
Upload And Activate Image
    ...
    ELSE
        ...
        VAR  ${activation_cmd}  busctl get-property org.open_power.Software.Host.Updater ${SOFTWARE_VERSION_URI}${version_id} xyz.openbmc_project.Software.Activation Activation | cut -d '"' -f 2
    ...
    Open Connection And Log In
    VAR  ${requested_activation_cmd}  busctl set-property org.open_power.Software.Host.Updater ${SOFTWARE_VERSION_URI}${version_id} xyz.openbmc_project.Software.Activation RequestedActivation s ${REQUESTED_ACTIVE}
    ${busctl_status}  ${busctl_out}=  Run Keyword And Ignore Error
    ...  BMC Execute Command  ${requested_activation_cmd}  ignore_err=${1}
    IF  '${busctl_status}' == 'PASS'
        ${output}  ${stderr}  ${rc}=  Set Variable  @{busctl_out}
        Should Be Equal As Integers  ${rc}  ${0}
        Should Be Empty  ${stderr}
    ...
    ...  Read Properties  ${SOFTWARE_VERSION_URI}${version_id}  timeout=${30}
    IF  '${status}' == 'PASS'
        ${requested_activation}=  Set Variable  ${software_state}[RequestedActivation]
    ELSE
        Open Connection And Log In
        VAR  ${requested_activation_cmd}  busctl get-property org.open_power.Software.Host.Updater ${SOFTWARE_VERSION_URI}${version_id} xyz.openbmc_project.Software.Activation RequestedActivation | cut -d '"' -f 2
...
Check Software Object Attribute
    ...
    IF  '${status}' != 'PASS'
        ...
        IF  '${sw_attribute}' == 'Priority'
            ...
            VAR  ${attribute_cmd}  busctl get-property org.open_power.Software.Host.Updater ${SOFTWARE_VERSION_URI}${image_object} ${dbus_iface} ${sw_attribute} | awk '{print $2}'
        ELSE
            VAR  ${dbus_iface}  xyz.openbmc_project.Software.Activation
            VAR  ${attribute_cmd}  busctl get-property org.open_power.Software.Host.Updater ${SOFTWARE_VERSION_URI}${image_object} ${dbus_iface} ${sw_attribute} | cut -d '"' -f 2
        END
...
Redfish Firmware Update Multipart
    [Documentation]  Perform  BMC/Host firmware update via update-multipart API.
    [Arguments]  ${image_path}  ${target}

    # Description of argument(s):
    # image_path   Path to the firmware image file to upload.
    # target       Target for which firmware to be updated(e.g. bmc,host)

    ${version_match}=  Compare Current Version And Image Version  ${image_path}  ${target}
    IF  not ${FORCE_UPDATE} and ${version_match}
        Pass Execution    The existing ${target} firmware is already active and FORCE UPDATE is ${FORCE_UPDATE}.
    END
    Print Timen  Uploading ${image_path} via update-multipart (target: ${target})...

    # Upload and get task ID directly from the 202 response.
    ${upload_resp}=  Update Multipart Image To BMC  ${image_path}  ${target}
    ...
    Print Timen  Firmware update task started: ${task_id}

    Wait Until Keyword Succeeds  10 min  5 sec
    ...  Verify Update Task Completed  ${task_id}
    ...
    ELSE IF  '${target}' == 'host'
        ...
        # Verify host firmware version matches uploaded image.
        Redfish Verify Host Version  ${image_path}
    ELSE
        Fail  msg=unknown target: ${target}
    END


Verify Update Task Completed
    [Documentation]  Poll the Task ID and verify both TaskState and TaskStatus.
    ...
    ${task_resp}=  Redfish.Get  ${REDFISH_TASKS_URI}/${task_id}
