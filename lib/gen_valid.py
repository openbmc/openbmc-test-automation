...
def process_error_message(error_message):
    ...
    For the following descriptive text, assume the caller of this function is
    a function named "valid_value()" which accepts 2 arguments: var_value,
    var_name.
...
additional_args_docstring_footer = \
    ...
    args                            Additional positional arguments (described
                                    below).
    kwargs                          Additional keyword arguements (described
                                    below).
    ...
    var_name                        The name of the variable whose value is
                                    ...
                                    this case, we are unable to get the
                                    variable name ourselves.
    """
...
def valid_range(var_value, lower=None, upper=None, *args, **kwargs):
    ...
    This function can be used with any type of operands where the can have a
...
def valid_integer(var_value, lower=None, upper=None, *args, **kwargs):
    r"""
    The variable value is valid if it is an integer.

    This function also calls valid_range to make sure the integer value is
    withing the specified range (if any).
    ...
    try:
        if isinstance(int(str(var_value), 0), int):
            var_name = get_var_name(*args, **kwargs)
            error_message = valid_range(var_value, lower, upper,
                                        var_name=var_name)
            return process_error_message(error_message)
    except ValueError:
        pass
...
def valid_list(var_value, valid_values=[], fail_on_empty=False, *args,
    ...
    Also, an empty list for var_value is considered invalid.
