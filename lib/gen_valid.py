...
def get_var_name(var_name):
    ...
    if var_name != "":
        return var_name
    # Calculate stack_frame_ix.  Determine whether a validation wrapper
    # function is calling.  For example, if the parent_func_name is
    # "svalid_value" and the grandparent_func_name is "valid_value" we have to
    # account for another layer in the call stack by incrementing the
    # stack_frame_ix.
    stack_frame_ix = 3
...
def svalid_value(var_value,
    ...
    invalid_values                  A list of invalid values.  If var_value is
                                    ...
                                    specify nothing both invalid_values and
                                    valid_values, invalid_values defaults to
                                    [""].
...
def valid_value(var_value,
                ...
                var_name=""):
    r"""
    If True if var_value is valid.  Otherwise, print an error message and
...
def valid_integer(var_value,
                  var_name=""):
    r"""
    If True if var_value is a valid integer.  Otherwise, print an error
...
def valid_dir_path(var_value,
                   var_name=""):
    r"""
    If True if var_value is a valid directory path.  Otherwise, print an error
    message and either return False or exit(1) depending on the value of
    exit_on_error.
...
def valid_file_path(var_value,
                    var_name=""):
    r"""
    If True if var_value is a valid file path.  Otherwise, print an error
    message and either return False or exit(1) depending on the value of
    exit_on_error.
...
def valid_range(var_value,
                ...
                var_name=""):
    r"""
    If True if var_value is within range.  Otherwise, print an error message
    and either return False or exit(1) depending on the value of exit_on_error.
