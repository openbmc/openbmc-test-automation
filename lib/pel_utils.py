...
def check_if_pel_transmitted_to_host(pel_id, expected_host_pel_trans_state="Acked"):
    r"""
    Return True if PEL is transmitted to Host else False.

    Description of arguments:
    pel_id                           PEL ID. E.g. 0x50000021.
    expected_host_pel_trans_state    Expected host transmission state for the PEL data.
                                     Default is "Acked".
    """
    ...
    try:
        ...
        if host_state != expected_host_pel_trans_state:
            return False
    except Exception as exception:
        raise PeltoolException(
