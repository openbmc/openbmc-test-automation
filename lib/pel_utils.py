...
def fetch_all_src():
    ...
    try:
        ...
        print("Fetch all SRC IDs: " + str(src_id))
    except Exception as e:
    ...
    try:
        ...
        for src in unexpected_src_list:
            if src in src_data:
                ...
                unknown_src_count = unknown_src_count + 1
        if (unknown_src_count >= 1):
            raise peltool_exception("Unexpected SRC found in " + str(unexpected_src_list))

    except Exception as e:
        raise peltool_exception("Failed to verify unexpected SRC list : " + str(e))
    return unknown_src_count
