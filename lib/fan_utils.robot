...
Get Target Speed Of Fans
    ...
    FOR    ${path}    IN    @{paths}
        ...
        IF    ${target_speed} > ${max_target}
            ${max_target}=    Set Variable    ${target_speed}
        ELSE
            ${max_target}=    Set Variable    ${max_target}
        END
    END
    RETURN    ${max_target}
...
Get Fan Target And Speed
    ...
    IF    ${clockwise_speed} > ${counterclockwise_speed}
        ${blade_speed}=    Set Variable    ${clockwise_speed}
    ELSE
        ${blade_speed}=    Set Variable    ${counterclockwise_speed}
    END
...
Verify Direct Fan Control
    ...
    IF    ${fail_test} > ${0}
        Fail    msg=hwmon did not properly report fan speeds.
    END
...
Verify Fan Speed Increase
    ...
    IF    ${initial_speed} < ${max_speed}
        ${expect_increase}=    Set Variable    1
    ELSE
        ${expect_increase}=    Set Variable    0
    END
    ...
    Rpvars    host_state
    IF    'Running' != '${host_state}'
        Pass Execution    msg=System shutdown so skipping remainder of test.
    END
