...
${IPMI_EXT_CMD} =       ipmitool -I lanplus -C 1 -P PASSW0RD -H
${RAW} =                raw
...
Run IPMI Command
    [arguments]    ${args}
    ${resp}=     Run Keyword If   '${IPMI_COMMAND}'=='External'   Run External IPMI RAW Command   ${args}
    ...          ELSE IF          '${IPMI_COMMAND}'=='Dbus'   Run Dbus IPMI RAW Command   ${args}

    [return]    ${resp}

Run IPMI Standard Command
    [arguments]    ${args}
    ${resp}=     Run Keyword If   '${IPMI_COMMAND}'=='External'   Run External IPMI Standard Command   ${args}
    ...          ELSE IF          '${IPMI_COMMAND}'=='Dbus'   Run Dbus IPMI Standard Command   ${args}
...
Run External IPMI RAW Command
    [arguments]    ${args}
    ${ipmi_raw_cmd}=   Catenate  SEPARATOR=    ${IPMI_EXT_CMD}${SPACE}${OPENBMC_HOST}${SPACE}${RAW}${SPACE}${args}
...
Run External IPMI Standard Command
    [arguments]    ${args}
    ${ipmi_cmd}=   Catenate  SEPARATOR=    ${IPMI_EXT_CMD}${SPACE}${OPENBMC_HOST}${SPACE}${args}
