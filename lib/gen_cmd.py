...
def shell_cmd(command_string,
    ...
    show_err                        If show_err is set, this function will
                                    print a standardized error report if the
                                    shell command fails (i.e. returns a
                                    shell_rc that is not in
    ...
    test_mode                       If test_mode is set, this function will
                                    ...
                                    print_output is set, it will print
                                    "(test_mode) Issuing: <cmd string>" to
                                    stdout.  When the test_mode argument is
                                    set to None, this function will assign a
                                    default value as follows:  The normal
                                    default is 0.  However, if this function
                                    is called via the alias t_shell_cmd, this
                                    program will search upward in the stack
                                    for the value of test_mode.  If no
                                    function in the stack (including the main
                                    module) has test_mode defined, it will be
                                    set to 0.  To summarize, a caller should
                                    use shell_cmd if they wish to have the
                                    command string run unconditionally.  They
                                    should use t_shell_cmd if they wish to run
                                    the command string only if the prevailing
    ...
    func_name = gp.sprint_func_name()
    called_func_name = gp.get_arg_name(0, 0, 2)
    if called_func_name == "t_" + func_name:
        # This function was called via the t_shell_cmd alias so get test_mode
        # from the call stack.
        test_mode = int(gp.get_var_value(test_mode,
                                         gp.get_stack_var('test_mode', 0)))
    else:
        # This function was called directly so set test_mode to zero if it is
        # None.
        test_mode = int(gm.dft(test_mode, 0))
...
# The t_shell_cmd alias is a shortcut for shell_cmd(test_mode=test_mode).  In
# other words, the command will only actually be run if test_mode is 0.
