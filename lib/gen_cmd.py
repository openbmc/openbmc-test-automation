...
def re_order_kwargs(stack_frame_ix, **kwargs):
    ...
    for arg_num in range(first_kwarg_pos, first_kwarg_pos + len(kwargs)):
        ...
        # From the prior example, key would be 'arg1'.
...
def create_command_string(command, *pos_parms, **options):
    ...
    by create_command_string and do NOT get inserted into the resulting
    command string.  All options following the arg_dashes/arg_delims options
    will use the specified values for dashes/delims.  In the special case of
    arg_dashes equal to "--", the arg_delim will automatically be changed to
    "=".  See examples below.

    Quoting rules:

    This function will single quote option values as needed to prevent bash
    expansion.  If the caller wishes to defeat this action, they may single or
    double quote the option value themselves.  See examples below.

    pos_parms are NOT automatically quoted.  The caller is advised to either
    add quotes or use the quote_bash_parm functions to quote any pos_parms.
    ...
    Note that the pos_parm ("~") does NOT get quoted.  If you wish to have it
    quoted, do the following:
    ...
    value.  To achieve this, use double quotes:
    ...
    if gp.is_dict(pos_parms[-1]):
        ...
        # Assign options the last pos_parm value (which is a dictionary).
        options = pos_parms[-1]
