...
def get_network_info(ip_version=4):
    ...
    Returns:
        Tuple of (mcahine_ip, subnet_cidr)
    """
    try:
        if ip_version == 4:
...
def are_in_same_network(host1, host2):
    ...
    try:
        ...
        is_ipv4_pair = isinstance(addr1, IPv4Address) and isinstance(addr2, IPv4Address)
        is_ipv6_pair = isinstance(addr1, IPv6Address) and isinstance(addr2, IPv6Address)
        ...
        # Use the more specific (larger prefix) subnet mask
