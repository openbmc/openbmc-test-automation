...
def wrapper_function_util(base_func_name,
                          wrap_func_name):

    r"""
    Return a function definition line and a base function call line.
    ...
    And the user wished to write a print_foo_bar wrapper function, they would
    call wrapper_function_util as follows:

    func_def_line, call_line = wrapper_function_util("sprint_foo_bar",
    "print_foo_bar")
    ...
    function.  The call_line is suitable for inclusion in the new wrapper
    function where it wishes to call the base function.  The benefit of these
    return values explicitly specifying each parm is that the caller of the
    wrapper function may then refer to any given parm by name when calling.
    ...
    # Get caller's module name.  Note: that for the present we've hard-code
    # the stack_frame_ix value because we expect a call stack to this function
    # to be something like this:
    # caller->create_print_wrapper_funcs->create_func_def_string->wrapper_funct
    # ion_util.
    stack_frame_ix = 3
...
def create_func_def_string(base_func_name,
    ...
    func_body_template              A function body in the form of a list.
                                    This is a template in so far as text
                                    substitions will be done on it to arrive
                                    at a valid function definition.  This
                                    template should NOT contain the function
                                    definition line (e.g. "def func1():").
                                    create_func_def_string will pre-pend that
                                    line for the caller.  The template should
                                    also contain the text "<call_line>" which
                                    is to be replaced by text which will call
                                    the base function with appropriate
                                    arguments.
    replace_dict                    A dictionary indicating what text
                                    replacements are to be done.  For example,
                                    if the template contains a "<sub1>" (be
                                    sure to include the angle brackets), and
                                    the dictionary contains a key/value pair
                                    of 'sub1'/'replace1', then all instances
                                    of "<sub1>" will be replaced by "replace1".
    """

    # Create the initial function defintion list as a copy of the template.
    func_def = list(func_body_template)
    # Call wrapper_function_util to get func_def_line and call_line.
    func_def_line, call_line = wrapper_function_util(base_func_name,
                                                     wrap_func_name)
    # Insert the func_def_line composed by wrapper_function_util is the first
    # list entry.
