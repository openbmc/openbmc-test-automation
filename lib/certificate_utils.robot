...
Generate Certificate File Via Openssl
    ...
    [Arguments]  ${cert_format}  ${time}=365  ${cert_dir}=cert_folder
    ...
    # cert_dir             Directory to store certificate.

    Check If Openssl Tool Exist

    ${openssl_cmd}=  Catenate  openssl req -x509 -sha256 -newkey rsa:2048
    ...  ${SPACE}-nodes -days ${time}
    ...  ${SPACE}-keyout ${cert_dir}/cert.pem -out ${cert_dir}/cert.pem
    ...  ${SPACE}-subj "/O=XYZ Corporation /CN=www.xyz.com"

    ${rc}  ${output}=  Run And Return RC and Output  ${openssl_cmd}
    Should Be Equal  ${rc}  ${0}  msg=${output}
    OperatingSystem.File Should Exist  ${EXECDIR}${/}${cert_dir}${/}cert.pem

    ${file_content}=  OperatingSystem.Get File  ${EXECDIR}${/}${cert_dir}${/}cert.pem
    ...
    ...  OperatingSystem.Get File  ${EXECDIR}${/}${cert_dir}${/}cert.pem
    ...
    Create File  ${cert_dir}/${cert_name}  ${cert_data}

    [Return]  ${EXECDIR}${/}${cert_dir}${/}${cert_name}
