...
def sprint_connections(connections=None,
    ...
    connection                      A list of connection objects which are
...
def execute_ssh_command(cmd_buf,
    ...
    open_connection_args            A dictionary of arg names and values which
                                    ...
                                    Of course, this are only needed if there
                                    is no open connection.  At a minimum, this
    ...
    for exec_cmd_attempt_num in range(1, max_exec_cmd_attempts + 1):
        ...
        try:
            stdout, stderr, rc = sshlib.execute_command(cmd_buf,
                                                        ...
                                                        return_stderr=True,
                                                        return_rc=True)
        except RuntimeError as execute_exception:
            if re.match(r"No open connection", str(execute_exception)):
                # There is no open connection so we must connect and login.
        ...
        except AssertionError as execute_exception:
            if re.match(r"Connection not open", str(execute_exception)):
                login_ssh(login_args)
                # Now we must continue to next loop iteration to retry the
                # execute_command.
                continue
            # We do not handle any other AssertionErrors so we will raise the
            # exception again.
            raise(execute_exception)
