...
def ipmi_cmd(cmd_string,
             ipmi_cmd_type='inband',
             print_output=1,
             ignore_err=0):
    r"""
    Run the given command string as an IPMI command return the stdout, stderr and the return code.

    # Description of argument(s):
    cmd_string                      The command string to be run as an IPMI command.
    ...
    ignore_err                      Ignore error means that a failure encountered by running the command
                                    string will not be raised as a python exception.
    """
...
def get_lan_print_dict(ipmi_cmd_type='external'):
    ...
    Description of argument(s):
    ipmi_cmd_type                   The type of ipmi command to use (e.g. 'inband', 'external').
    """
    ...
    stdout1, stderr, rc = ipmi_cmd(cmd_buf, ipmi_cmd_type, print_output=0)
    ...
    stdout2, stderr, rc = ipmi_cmd(cmd_buf, ipmi_cmd_type, print_output=0)
