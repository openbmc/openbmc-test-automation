...
def build_ipmi_ext_cmd(quiet=None):
    ...
    This keyword is designed for use by keywords which is the IPMI_EXT_CMD
    variable (e.g. 'Run External IPMI Raw Command').  This keyword is
    warranted because the ipmitool program may or may not accept the -U
    ...
    ipmi_ext_cmd = BuiltIn().get_variable_value("${IPMI_EXT_CMD}", "root")
    if ipmi_ext_cmd != "":
        return

    quiet = int(gp.get_var_value(quiet, 0))
    openbmc_host = BuiltIn().get_variable_value("${OPENBMC_HOST}", "root")
    ...
    ipmi_cmd_suffix = " -H " + openbmc_host + " raw 6 1"
    ...
    if old_rc != 0 and new_rc != 0:
        BuiltIn().fail("Unable to run ipmitool, (with or without the '-U' parm).")
