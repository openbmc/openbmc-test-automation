...
def build_test_attribute_registry(
    registry_id: str = "BiosAttributeRegistry",
) -> dict:
...
def compare_attribute_registries(expected: dict, actual: dict) -> list[str]:
    ...
    actual_by_name = {
        attribute["AttributeName"]: attribute
        for attribute in actual.get("RegistryEntries", {}).get(
            "Attributes", []
        )
    }
    ...
    for name, expected_attribute in expected_by_name.items():
        ...
        if actual_attribute is None:
            mismatches.append(
                f"Attribute '{name}' missing from read-back registry."
            )
            continue
    ...
    for name in actual_by_name.keys() - expected_by_name.keys():
        mismatches.append(
            f"Attribute '{name}' unexpectedly present in read-back registry."
        )
