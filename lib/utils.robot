...
Verify Identify LED State
    [Documentation]  Verify the identify LED state. Check the LED group instead of physical LED
    ...  matches caller's expectations.
    [Arguments]  ${expected_state}

    # Description of argument(s):
    # expected_state  The LED asserted state expected by the caller (1 is asserted, 0 is not asserted)

    ${resp}=  Get LED State XYZ  enclosure_identify
