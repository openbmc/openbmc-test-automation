...
Initiate OS Host Reboot
    ...
    IF    '${os_username}' == 'root'
        ${cmd_buf}=    Set Variable    reboot
    ELSE
        ${cmd_buf}=    Set Variable    echo ${os_password} | sudo -S reboot
    END
...
Initiate OS Host Power Off
    ...
    IF    ${hard}
        ${time_string}=    Set Variable    ${SPACE}now
    ELSE
        ${time_string}=    Set Variable    ${EMPTY}
    END

    IF    '${os_username}' == 'root'
        ${cmd_buf}=    Set Variable    shutdown${time_string}
    ELSE
        ${cmd_buf}=    Set Variable    echo ${os_password} | sudo -S shutdown${time_string}
    END
