...
Get Boot Progress
    ...
    ${state} =  Read Attribute
    ...  ${OPENBMC_BASE_URI}sensors/host/BootProgress  value  quiet=${quiet}
    [return]  ${state}
...
Initiate Power On
    ...
    ${resp}=   Call Method
    ...    ${OPENBMC_BASE_URI}control/chassis0/   powerOn  data=${args}
...
Initiate Power Off
    ...
    ${args}=     Create Dictionary    data=@{arglist}
    ${resp}=   Call Method
    ...  ${OPENBMC_BASE_URI}control/chassis0/    powerOff  data=${args}
    should be equal as strings      ${resp.status_code}     ${HTTP_OK}
...
Clear BMC Record Log
    ...
    @{arglist}=   Create List
    ${args}=     Create Dictionary    data=@{arglist}
    ${resp}=   Call Method    ${OPENBMC_BASE_URI}records/events/  clear
    ...  data=${args}
    should be equal as strings      ${resp.status_code}     ${HTTP_OK}
