...
Smart Combine Lists
    ...
    # args  Arguments which are to be passed to the Combine Lists keyword.
    #       Any argument which is not a valid list will be removed.

    ${list_size}=  Get Length  ${args}
    ${index}  Set Variable  ${0}
    : FOR  ${arg}  IN  @{args}
    \    ${type_arg}=  Evaluate  str(type($args[${index}])).split("'")[1]
    \    Run Keyword If  '${type_arg}' != 'list'  Run Keywords
    ...      Remove From List  ${args}  ${index}  AND
    ...      Continue For Loop
    \    ${index}=  Evaluate  ${index}+1

    ${new_list}=  Combine Lists  @{args}
