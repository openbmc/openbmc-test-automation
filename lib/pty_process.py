...
def _require_pexpect():
    if not _PEXPECT_AVAILABLE:
        raise RuntimeError(
            "The 'pexpect' package is required by PtyProcess but is not "
            "installed.  Install it with:  pip install pexpect"
        )


class PtyProcess:
    """Robot Framework library for running interactive commands in a PTY.
    ...
    def start_pty_process(self, command, log_file, alias=None,
                          encoding="utf-8", timeout=30, dimensions=(24, 200)):
        """Start *command* inside a pseudo-terminal and stream all output to *log_file*.
        ...
        log_fh = open(log_file, "w", encoding=encoding, errors="ignore", buffering=1)
        ...
        logger.info("[PtyProcess] Started PTY process alias='{}': {}".format(alias, command))
        logger.info("[PtyProcess] Session log: {}".format(log_file))
        return alias

    def stop_pty_process(self, alias, force=False):
        """Stop the PTY process identified by *alias* and close its log file.
        ...
        if child is None:
            logger.warn("[PtyProcess] No PTY process found with alias '{}'".format(alias))
            return -1
        ...
        try:
            ...
            logger.warn("[PtyProcess] Error stopping '{}': {}".format(alias, exc))
        ...
        logger.info("[PtyProcess] Stopped PTY process '{}', exit_code={}".format(alias, exit_code))
        return exit_code

    def stop_all_pty_processes(self, force=True):
        """Stop **all** running PTY processes.  Intended for suite/test teardown.
    ...
    def send_to_pty_process(self, alias, text):
        """Send *text* to the stdin of the running PTY process (no newline appended).
    ...
    def send_line_to_pty_process(self, alias, text):
        """Send *text* followed by a newline to the PTY process stdin.
        ...
        child = self._get_process(alias)
        child.sendline(text)
        logger.info("[PtyProcess] Sent line to '{}': {}".format(alias, repr(text)))

    # ------------------------------------------------------------------
    # Output / wait keywords
    # ------------------------------------------------------------------

    def wait_for_pty_process_output(self, alias, pattern, timeout=30.0,
                                     error_patterns=None):
        """Wait until *pattern* (regex) appears in the PTY process output.
        ...
        if idx == 0:
            ...
            logger.info("[PtyProcess] '{}' matched pattern: {}".format(alias, repr(matched)))
            return matched
        elif 1 <= idx < n_user:
    ...
    def read_pty_process_output(self, alias, timeout=2.0):
        """Read any output currently available from the PTY process.
        ...
        child = self._get_process(alias)
        try:
            child.expect([pexpect.TIMEOUT, pexpect.EOF], timeout=float(timeout))
        except pexpect.ExceptionPexpect:
            pass
        output = child.before or ""
        logger.info("[PtyProcess] Read from '{}' ({} chars): {}".format(
    ...
    def is_pty_process_running(self, alias):
        """Return ``True`` if the PTY process *alias* is still alive.
    ...
    def wait_for_pty_process_to_exit(self, alias, timeout=30.0):
        ...
        try:
            ...
            raise RuntimeError(
                "[PtyProcess] Process '{}' did not exit within {}s".format(alias, timeout)
            )
        exit_code = child.wait() if child.isalive() else (child.exitstatus or 0)
        logger.info("[PtyProcess] '{}' exited with code {}".format(alias, exit_code))
        return exit_code
    ...
    def get_pty_process_log(self, log_file):
        """Read and return the full content of a PTY session log file.
        ...
        try:
            with open(log_file, "r", encoding="utf-8", errors="ignore") as fh:
                content = fh.read()
            logger.info("[PtyProcess] Read log '{}': {} bytes".format(log_file, len(content)))
            return content
    ...
    def get_pty_process_log_path(self, alias):
        """Return the log file path associated with *alias*.
    ...
    # ------------------------------------------------------------------
    # SOL convenience keywords
    # ------------------------------------------------------------------

    def start_sol_pty_session(self, ipmi_command, log_file, alias="sol_pty",
                               startup_timeout=30.0,
                               startup_pattern=r"SOL Session operational"):
        """Start an ipmitool SOL session in a PTY and wait for the operational banner.
        ...
        try:
            ...
            raise RuntimeError(
                "SOL session '{}' did not become operational: {}".format(alias, exc)
            )
        logger.info("[PtyProcess] SOL session '{}' is operational".format(alias))
        return alias

    def resume_pty_process(self, alias):
        """Send SIGCONT to a suspended PTY process to resume it.
        ...
        try:
            ...
            logger.info(
                "[PtyProcessLib] Sent SIGCONT to '{}' (pid={})".format(alias, child.pid)
            )
        except Exception as exc:
            logger.warn(
                "[PtyProcessLib] Error sending SIGCONT to '{}': {}".format(alias, exc)
            )

    def stop_sol_pty_session(self, alias="sol_pty", exit_sequence="~.",
                              exit_timeout=15.0):
        """Terminate an ipmitool SOL session by sending the ``~.`` escape sequence.
    ...
    # ------------------------------------------------------------------
    # Internal helpers (not exposed as keywords – start with _)
    # ------------------------------------------------------------------

    def _get_process(self, alias):
        """Return the pexpect child for *alias*, raising if not found."""
        with self._lock:
