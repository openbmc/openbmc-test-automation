...
Get The Dump Id
    ...
    IF    ${resp.json()["data"]} == ${None}    Fail    Dump id returned null.
...
Wait For Task Completion
    ...
    FOR    ${retry}    IN RANGE    ${retry_max_count}
        ...
        IF    ${check_state} == ${TRUE}
            Should Be True    '${resp["TaskState"]}' in ${allowed_task_state}    msg=Verify task state is valid
        END
