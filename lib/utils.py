...
print('0. code_base_dir_path : %s' %code_base_dir_path, end='\n')
...
def get_json_data(file_path=""):
    ...
    file_path                       The path to the json file under the package. The execution path will
                                    be obtained from the "BOOT_TABLE_PATH" environment variable.
                                    If this value is a relative path, this function will use
                                    the code_base_dir_path as the base directory (see definition above).
    """

    print('1. File Path : %s' %file_path, end='\n')
    #if file_path is not None:
    #    file_path = os.environ.get('BOOT_TABLE_PATH', file_path)
    print('2. File Path : %s' %file_path, end='\n')
    if not file_path.startswith("/"):
        file_path = code_base_dir_path + file_path
    print('3. File Path : %s' %file_path, end='\n')
    print('4. code_base_dir_path : %s' %code_base_dir_path, end='\n')
