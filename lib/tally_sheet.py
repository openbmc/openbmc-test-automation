...
class tally_sheet:
    ...
    def __init__(self,
        ...
        self.row_key_field_name = row_key_field_name
        ...
        self.init_fields_dict = init_fields_dict
        self.totals_line = init_fields_dict
        self.sum_fields = []
        self.calc_fields = []
    ...
    def add_row(self, row_key, init_fields_dict=None):
        ...
        if init_fields_dict is None:
            init_fields_dict = self.init_fields_dict
        try:
            self.table[row_key] = collections.OrderedDict(init_fields_dict)
        except AttributeError:
            self.table[row_key] = DotDict(init_fields_dict)
    ...
    def update_row_field(self, row_key, field_key, value):
        ...
        self.table[row_key][field_key] = value
    ...
    def inc_row_field(self, row_key, field_key):
        ...
        self.table[row_key][field_key] += 1
    ...
    def calc(self):
        ...
        self.totals_line = copy.deepcopy(self.init_fields_dict)
        # Walk through the rows of the table.
        for row_key, value in self.table.items():
            # Walk through the calc fields and process them.
            for calc_field in self.calc_fields:
                ...
                for token in tokens:
                    if token in ("=", "+", "-", "*", "/"):
                        ...
                        cmd_buf += "self.table['" + row_key + "']['" +\
                                   token + "'] "
                exec(cmd_buf)

            for field_key, sub_value in value.items():
                if field_key in self.sum_fields:
                    self.totals_line[field_key] += sub_value

        return self.totals_line
    ...
    def sprint_obj(self):
        ...
        buffer = ""
        buffer += gp.sprint_var(self.obj_name)
        buffer += gp.sprint_var(self.row_key_field_name)
        buffer += gp.sprint_var(self.table)
        buffer += gp.sprint_var(self.init_fields_dict)
        buffer += gp.sprint_var(self.sum_fields)
        buffer += gp.sprint_var(self.totals_line)
        buffer += gp.sprint_var(self.calc_fields)
        buffer += gp.sprint_var(self.table)
        return buffer
    ...
    def sprint_report(self):
        ...
        col_names = [self.row_key_field_name.title()]
        report_width = 30
        key_width = 30
        ...
        first_rec = next(iter(self.table.items()))
        ...
        buffer += format_string.format('Totals', *self.totals_line.values()) +\
            "\n"
