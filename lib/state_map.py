...
if platform_arch_type == "x86":
    ...
    for state_name, state_set in VALID_BOOT_STATES.items():
        ...
        for state_tuple in state_set:
            state_tuple_new = tuple(x for x in state_tuple
                                   if (not (x.startswith("xyz.openbmc_project.State.Boot.Progress") or
                                            x.startswith("xyz.openbmc_project.State.OperatingSystem"))))
