...
Get SNMP URI List
    [Documentation]  Get all SNMP URIs.
...
Configure SNMP Manager
    [Documentation]  Configure SNMP manager.
...
Verify SNMP Manager
    [Documentation]  Verify SNMP manager.
...
Delete SNMP Manager And Object
    ...
    \  Run Keyword If  "${snmp_ip}" == "${ip}" and "${port}" == "${port1}"  Exit For Loop
