...
# Once the State Manager support is ready move it out to appropriate
# resource or variable file
...
Initiate Power On XYZ 
    [Documentation]  Initiates host power on.
    ${arglist}=   Set Variable   ${HOST_POWERON_TRANS}
...
Initiate Power Off XYZ
    [Documentation]  Initiates host power off.
    ${arglist}=   Set Variable   ${HOST_POWEROFF_TRANS}
    ${args}=     Create Dictionary    data=${arglist}
    Write Attribute XYZ
    ...  ${HOST_STATE_URI}  RequestedHostTransition   data=${args}
...
Get Host State XYZ
    [Documentation]  Returns the state of the host as a string.
    ${state}=
...
Get Chassis Power State XYZ
    [Documentation]  Returns the  power state of the Chassis
    ...              as a string
    ${state}=
