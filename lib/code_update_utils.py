...
def verify_image_upload(image_version, timeout=3):
    ...
    via REST (timeout=30s, errors ignored).  If the REST read fails the loop
    sleeps 30s and retries.

    Note: REST read failures consume an iteration slot in the same way as a
    successful read that returns an unexpected state.  With the default
    timeout=3 (6 total iterations), up to 6 transient REST failures will
    exhaust the retry budget and return False even if the D-Bus object
    registers on a subsequent attempt.  Increase timeout if the hardware is
    known to have intermittent REST availability during image registration.
    ...
    timeout                         How long, in minutes, to keep retrying.
                                    ...
                                    Note that both successful reads with an
                                    unexpected state AND REST read failures
                                    each consume one iteration.
    """
    ...
    if (
        ...
        for _ in range(timeout * 2):
            ...
            if status != "PASS":
                BuiltIn().log_to_console(
                    f"verify_image_upload REST read failed, retrying: {image_version_id}"
                )
                time.sleep(30)
                continue
