...
def wait_for_activation_state_change(version_id, initial_state):
    ...
    REST reads use a 30-second socket timeout.  If the REST read fails
    (e.g. during a flash write that stalls bmcweb), the function falls back
    to a direct busctl SSH call against both the Host and BMC updater
    services.  Consecutive read failures are tolerated up to
    read_fail_threshold (3) before the test is failed.  The polling loop
    runs up to 60 iterations with a 10-second sleep between each successful
    read and a 30-second sleep after every failed read.
...
def verify_image_upload(image_version, timeout=3):
    ...
    between iterations.  On each iteration the Activation attribute is first
    read via REST (timeout=30s, errors ignored).  If the REST read fails the
    function falls back to a direct busctl SSH call against the appropriate
    updater service (Host or BMC) before sleeping and retrying.
    ...
    timeout                         How long, in minutes, to keep retrying.
                                    Each minute covers 2 iterations at 30s
                                    sleep.  Default is 3 minutes (6 attempts).
    """
