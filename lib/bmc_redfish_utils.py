...
class bmc_redfish_utils(object):
    ...
    def list_request(self, resource_path):
        ...
        if not self.__resource_list:
            return uri_path
        for resource in self.__resource_list.copy():
            self._rest_response_ = \
            ...
            if self._rest_response_.status != 200:
                continue
            self.walk_nested_dict(self._rest_response_.dict)
        return list(sorted(self.__resource_list))
    ...
    def enumerate_request(self, resource_path):
        ...
        while resources_to_be_enumerated:
            for resource in resources_to_be_enumerated:
                ...
                self.walk_nested_dict(self._rest_response_.dict, resource, True)
    ...
    def walk_nested_dict(self, data, url='', enumeration=False):
        r"""
        ...
        enumeration Set it to True to populate variables used in enumerate_request
                    method.
        """
                    ...
                    if isinstance(value, list):
                        for memberDict in value:
                            if enumeration:
                                ...
                                self.__resource_list.add(memberDict['@odata.id'])
                if '@odata.id' == key:
                    value = value.rstrip('/')
                    if enumeration:
                        ...
                        self.__resource_list.add(value)
