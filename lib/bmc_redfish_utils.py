...
class bmc_redfish_utils(object):
    ...
    def get_valid_path_properties(self, resource_path, attribute):
        r"""
        Returns the list of valid path which has a given attribute.

        Description of argument(s):
        resource_path_list        URI resource base path (e.g.
                                  '/redfish/v1/Systems/',
                                  '/redfish/v1/Chassis/').
        ...
        for path_idx in resource_path_list:
            ...
            for child_path_idx in child_path_list:
                resp = self.get_attribute(child_path_idx, attribute)
                if resp:
