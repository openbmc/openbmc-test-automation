...
Redfish Create User
    ...
    IF    ${force} == ${True}
        Redfish.Delete    ${REDFISH_ACCOUNTS_URI}${user_name}    valid_status_codes=[${HTTP_OK}, ${HTTP_NOT_FOUND}]
    END
    ...
    IF    ${force} == ${True} or ${user_exists} == ${False}
        Redfish.Post    ${REDFISH_ACCOUNTS_URI}    body=&{payload}    valid_status_codes=[${HTTP_CREATED}]
    END
