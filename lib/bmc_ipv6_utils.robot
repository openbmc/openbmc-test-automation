...
Verify IPv6 On BMC
    ...
    [Arguments]  ${ipv6}  ${channel_number}=${CHANNEL_NUMBER}
...
Set DHCPv6 Property
    [Documentation]  Set DHCPv6 attribute to enabled or disabled.
    [Arguments]  ${dhcpv6_operating_mode}=Disabled  ${channel_number}=${CHANNEL_NUMBER}
    ...
    IF  '${dhcpv6_operating_mode}' == 'Disabled'
        VAR  ${data}  ${DISABLE_DHCPv6}
    ELSE
        VAR  ${data}  ${ENABLE_DHCPv6}
    END
    ${active_channel_config}=  Get Active Channel Config
...
Get IPv6 SLAAC Enabled Status
    [Documentation]  Return IPv6 SLAAC enabled status from redfish URI.
    [Arguments]  ${channel_number}=${CHANNEL_NUMBER}
    ...
    VAR  ${ethernet_interface}  ${active_channel_config['${channel_number}']['name']}
    ${resp}=  Redfish.Get  ${REDFISH_NW_ETH_IFACE}${ethernet_interface}
    ${slaac_config}=  Get From Dictionary  ${resp.dict}  StatelessAddressAutoConfig

    RETURN  ${slaac_config['IPv6AutoConfigEnabled']}


Get DHCPv6 Operating Mode
    [Documentation]  Return DHCPv6 operating mode from redfish URI.
    [Arguments]  ${channel_number}=${CHANNEL_NUMBER}
    ...
    VAR  ${ethernet_interface}  ${active_channel_config['${channel_number}']['name']}
    ${resp}=  Redfish.Get  ${REDFISH_NW_ETH_IFACE}${ethernet_interface}
    ${dhcpv6_config}=  Get From Dictionary  ${resp.dict}  DHCPv6

    RETURN  ${dhcpv6_config['OperatingMode']}
