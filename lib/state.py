...
def get_os_state(os_host="",
    ...
    Description of arguments:
    os_host      The DNS name or IP address of the operating system host.
                 This defaults to global ${OS_HOST}.
    os_username  The username to be used to login to the host.
                 This defaults to global ${OS_USERNAME}.
    os_password  The password to be used to login to the host.
    ...
    if login:
        # Try running a simple command (uptime) on the host.
    ...
    host_state = BuiltIn().run_keyword(*cmd_buf)
...
def get_state(openbmc_host="",
    ...
    Description of arguments:
    openbmc_host      The DNS name or IP address of the open BMC system host.
                      This defaults to global ${OPENBMC_HOST}.
    openbmc_username  The username to be used to login to the open BMC host.
                      This defaults to global ${OPENBMC_USERNAME}.
    openbmc_password  The password to be used to login to the host.
    ...
    # Set parm defaults where necessary and validate all parms.  NOTE: OS host
    # parms are optional.
    if os_host == "":
        os_host = BuiltIn().get_variable_value("${os_host}")
        if os_host is None:
    ...
    if os_host != "":
        # Create a host_match dictionary to test whether we are booted enough
        # to get operating system info.
        cmd_buf = ["Create Dictionary", "power=^${1}$", "bmc=^HOST_BOOTED$",
                   "boot_progress=^FW Progress, Starting OS$"]
        grp.rdpissuing_keyword(cmd_buf)
        host_match = BuiltIn().run_keyword(*cmd_buf)
        host_match = compare_states(state, host_match)
        ...
        if host_match:
            ...
            for key, state_value in os_state.items():
                # Add each host value to the state dictionary, pre-pending
                # "os_" to each key.
                new_key = "os_" + key
