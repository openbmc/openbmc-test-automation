...
def init_robot_out_parms(extra_prefix=""):
    ...
    # Environment variable TMP_ROBOT_DIR_PATH can be set be the user to
...
def process_robot_output_files(robot_cmd_buf,
                               robot_rc,
                               gzip=1):
    r"""
    Process robot output files which can involve several options:
    - If they're in a temporary location, deciding whether to move them to the
      permanent location.
    - Gzipping them.
    - Deleting them if policy is FAIL and robot_rc is 0.
    Description of argument(s):
    robot_cmd_buf                   The complete command string used to invoke
                                    robot.
    robot_rc                        The return code from running robot.
    ...
    # STATUS_DIR_PATH.  Otherwise, we're done.

    if os.environ.get("TMP_ROBOT_DIR_PATH", "") is "":
        return

    # We're directing these to the FFDC/STATUS dir path so that they'll be
    # sujected to FFDC cleanup.
...
def robot_cmd_fnc(robot_cmd_buf,
    ...
    process_robot_output_files(robot_cmd_buf, shell_rc)
    if shell_rc != 0:
        hex = 1
