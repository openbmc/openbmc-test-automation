...
def sprint_varx(var_name,
    ...
    trailing_char                   The character to be used at the end of the
                                    ...
                                    line feed.
    key_list                        Note: this argument pertains only to
                                    var_values which are dictionaries.  A list
                                    of keys which should be printed.  All
                                    others keys will be skipped.  Each value
                                    in key_list will be regarded as a regular
                                    expression and it will be regarded as
                                    anchored to the beginning and ends of the
                                    dictionary key being referenced.  For
                                    example if key_list is ["one", "two"], the
                                    resulting regex used will be "^one|two$",
                                    i.e. only keys "one" and "two" from the
                                    var_value dictionary will be printed.  As
                                    another example, if the caller were to
                                    specify a key_list of ["one.*"], then only
                                    dictionary keys whose names begin with
                                    "one" will be printed.
    """

    if loc_col1_indent is None or loc_col1_indent == "":
        loc_col1_indent = col1_indent
    if loc_col1_width is None or loc_col1_width == "":
        loc_col1_width = col1_width
    if trailing_char is None or trailing_char == "":
        trailing_char = "\n"
