...
These functions support automated sensor validations as part of the Robot Framework
tests in an OpenBMC environment.
"""
...
def validate_threshold_values(sensor_threshold_values, sensor_id):
    ...
    :param sensor_threshold_values: Dictionary of threshold values.
    :param sensor_id: Sensor identifier (e.g., "fan_1").
    """
    thresholds = ["lnr", "lcr", "lnc", "unc", "ucr", "unr"]
    
    # Validate lower thresholds
    if not (float(sensor_threshold_values["lnr"]) < 
            float(sensor_threshold_values["lcr"]) < 
            float(sensor_threshold_values["lnc"])):
        error_msg = (f"{sensor_id}: Lower thresholds violate IPMI spec\n"
                     f"lnr={sensor_threshold_values['lnr']}, "
                     f"lcr={sensor_threshold_values['lcr']}, "
                     f"lnc={sensor_threshold_values['lnc']}")
        BuiltIn().fail(gp.sprint_error(error_msg))

    # Validate upper thresholds
    if not (float(sensor_threshold_values["unc"]) > 
            float(sensor_threshold_values["ucr"]) > 
            float(sensor_threshold_values["unr"])):
        error_msg = (f"{sensor_id}: Upper thresholds violate IPMI spec\n"
...
def check_reading_value_length(sensor_reading, sensor_id, sensor_unit):
    ...
    :param sensor_reading: Reading value (e.g., "1234.567").
    :param sensor_id: Sensor identifier (e.g., "temp_ambient").
    :param sensor_unit: Sensor unit (e.g., "RPM").
    """
    ...
    if len(integer_part) > max_int_len:
        BuiltIn().fail(gp.sprint_error(
            f"{sensor_id}: Integer part exceeds {max_int_len} digits ({integer_part})"
        ))

    if len(fractional_part) > max_frac_len:
        BuiltIn().fail(gp.sprint_error(
            f"{sensor_id}: Fractional part exceeds {max_frac_len} digits ({fractional_part})"
        ))
...
def convert_sensor_name_as_per_ipmi_spec(sensor_name):
    ...
    :param sensor_name: Original sensor name.
    :return: Truncated/padded 16-byte string.
    """
...
def create_sensor_list_not_having_single_threshold(ipmi_sensor_response, threshold_sensor_list):
    ...
    :param ipmi_sensor_response: Raw IPMI sensor output.
    :param threshold_sensor_list: List of expected threshold sensors.
    :return: List of sensors missing thresholds.
    """
