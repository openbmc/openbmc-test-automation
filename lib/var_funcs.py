...
def match_struct(structure, match_dict, regex=False):
    r"""
    Return True or False to indicate whether the given structure matches the
    given the match dictionary.
    ...
    match_dict                      Each key/value pair in match_dict must
                                    ...
                                    special case.  In this case, the structure
                                    would be considered a match only if the
                                    key in question is found nowhere in the
                                    structure.
    ...
    # The structure must pass all tests to be considered a match.  Therefore,
    # any failure to match is grounds for returning False.
    for match_key, match_value in match_dict.items():
        # Process one match key/value pair.
        ...
        if match_value is None:
            ...
            if len(struct_key_values) == 0:
                return False
            # If any of the struct_key_values match we have a valid match so
            # we'll continue to the next match_dict entry.
            if regex:
