...
Continuous Node Power Status
    [Documentation]  Continuously get the power status, it verifies the
    ...  REST connectivity between XCAT and BMC node.
...
Get Temperature Reading Through XCAT
    [Documentation]  Get temperature reading through XCAT.
    [Tags]  Get_Temperature_Reading_Through_XCAT

    # Perform this operation on one BMC node.
    ...
    ${temp_reading}=  Get Rvital Data Through XCAT  ${BMC_LIST[1]}  temp
    Log  ${temp_reading}

Get Fanspeed Through XCAT
    [Documentation]  Get fanspeed through XCAT.
    [Tags]  Get_Fanspeed_Reading_Through_XCAT
    ...
    ${fanspeed}=  Get Rvital Data Through XCAT  ${BMC_LIST[1]}  fanspeed
    Log  ${fanspeed}

Get Voltage Reading XCAT
    [Documentation]  Get voltage through XCAT.
    [Tags]  Get_Voltage_Through_XCAT
    ...
    ${voltage}=  Get Rvital Data Through XCAT  ${BMC_LIST[1]}  voltage
    Log  ${voltage}

Get Wattage Through XCAT
    [Documentation]  Get wattange through XCAT.
    [Tags]  Get_Wattage_Through_XCAT
    ...
    ${wattage}=  Get Rvital Data Through XCAT  ${BMC_LIST[1]}  wattage
    Log  ${wattage}
