...
Verify Add SEL Entry
    ...
    # Original keyword "Create User Define SEL" for reference
    Create Test PEL Log
...
Verify Reservation ID Erasure Status
    ...
    ...  whether erasure status updated in clear sel command response data using new Reserve ID`.
...
Create User Define SEL
    [Documentation]  Create a user define tempearature sensor SEL.
...
Get SEL Entry Via IPMI
    ...
    # For example, when a first sel entry is added with IPMI raw command, the reseponse will be "01 00".
