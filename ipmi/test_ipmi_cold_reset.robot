...
Cold Reset Via IPMI
    ...
    # Wait to see that BMC was first shutdown and then fully rebooted.
...
Verify Cold Reset Impact On Sensor Threshold Via IPMI
    ...
    ${threshold_value_before_reset}=  Get From Dictionary  ${sensor_threshold}  ${random_threshold_key}

    # Modify default threshold value for a sensor to set an new threshold value
    ${new_settable_threshold_value}=  Modify Default Sensor Threshold Value  ${threshold_value_before_reset}
    ...
    Should Not Be Equal  ${new_threshold_value}  ${threshold_value_before_reset}
    ...
    ${threshold_value_after_reset}=  Get Sensor Threshold Value  ${random_threshold_key}  ${sensor_name}

    # Compare with initial sensor threshold values.
    Should Be Equal  ${threshold_value_after_reset}  ${threshold_value_before_reset}
...
Get The Sensor Name And Threshold
    [Documentation]  Get the sensor threshold for given sensor using IPMI.
    [Arguments]  ${sensor_list}

    # Description of Argument(s):
    #    sensor_list    Sensors listed in ipmi sensor list command.

    @{tmp_list}=  Create List

    @{sensor_list_lines}=  Split To Lines  ${sensor_list}

    # Omit the discrete sensor and create a threshold sensor name list.
    ...
    FOR  ${RANGE}  IN RANGE  0  ${sensor_count}
      ${random_sensor}  ${sensor_threshold}=  Select Random Sensor Name And Threshold Value  ${tmp_list}  ${sensor_list}
...
Select Random Sensor Name And Threshold Value
    [Documentation]  Random sensor will be selected from sensor name list 
    ...  and default threshold value for that random sensor will returned in dict.
    [Arguments]  ${tmp_list}  ${sensor_list}

    # Description of Argument(s):
    #    tmp_list      Sensors name list which have only threshold based sensor names.
    #    sensor_list   Sensors listed in ipmi sensor list command.
...
Modify Default Sensor Threshold Value
    [Documentation]  Modify the default sensor threshold by adding 100 to the old threshold.
    [Arguments]  ${default_threshold}

    # Description of Argument(s):
    #    default_threshold    Default threshold value for an sensor.

    ${new_threshold}=  Evaluate  ${default_threshold} + 100
...
Set And Get Sensor Threshold For Given Sensor
    ...
    # Description of Argument(s):
    #    sensor_name                     Name of the sensor which threshold value is going to be changed.
    #    random_threshold_key            Name of threshold keys(lnc, lcr, lnr, unc, ucr and unr) to be changed.
    #    new_settable_threshold_value    New threshold value needs to be changed for an sensor.
    ...
    ${sensor_new_threshold_value}=  Get Sensor Threshold Value  ${random_threshold_key}  ${sensor_name}

    [Return]  ${sensor_new_threshold_value}

Get Sensor Threshold Value
    [Documentation]  Get Particular Sensor Threshold Value Based On Sensor Name And Threshold Key.
    [Arguments]  ${threshold_key}  ${sensor_name}

    # Description of Argument(s):
    #    sensor_name              Name of the sensor for getting threshold value.
    #    threshold_key            Name of the particular threshold key that value needs to be get.

    # Get New Sensor Threshold Value.
