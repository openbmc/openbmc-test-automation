...
Check Expected Sensors Are Showing In IPMI Sensor
    ...
    ${expected_sensors_not_listing_count}=  Get Length  ${expected_sensors_not_listing_in_ipmi}

    Run Keyword If  ${expected_sensors_not_listing_count} != 0
    ...  Run Keywords  Log  ${expected_sensors_not_listing_in_ipmi}  AND
    ...  Fail  message=${expected_sensors_not_listing_count} expected sensors are not showing in ipmi sensor.
...
Suite Setup Execution
    [Documentation]  Create an dbus uri dictionary, sensor list which was not mapped in sensor tree
    ...  and threshold sensor name list.
    ...
    # create a sensor_details.py python file on openbmc-test-automation/data directory.
...
Compare Threshold Values For Fan
    ...
    ...  message= Threshold Value Was Showing Wrongly, In DBus - ${dbus_value} and IPMI - ${ipmi_value}.
