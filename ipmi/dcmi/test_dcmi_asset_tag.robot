...
Validate Asset Tag Via Raw Command
    ...
    ${cmd_rsp}=  Get Raw Asset Tag
    @{rsp_list}=  Split String  ${cmd_rsp}
    Run Keyword And Continue On Failure
    ...  Valid Value  rsp_list[1]  valid_values=['${number_of_bytes_to_write}']
    ${data_list}=  convert_prefix_hex_list_to_non_prefix_hex_list  ${string_hex_list}
    Lists Should Be Equal  ${data_list}  ${rsp_list[2:]}
