...
Get Available Sensors
    ...
    ...                Returns a list available sensors(e.g.:['dcm0_cpu0', 'dcm0_cpu1', 'dcm1_cpu0', 'dcm1_cpu1']).
    ...
    FOR  ${id}  IN  @{sensors}
        ${ids}=  Set Variable  ${id.split('|')[0].strip()}
        ${value}=  Set Variable  ${id.split('|')[1].strip()}
        ${contains}=  Evaluate  """na""" in "${value}"
        Run Keyword IF  "${contains}" != """True"""  Append To List  ${sensor_list}  ${ids}
    END
    Log To Console  Available sensors: ${sensor_list}
    [RETURN]  ${sensor_list}


Get IPMI Sensor ID Dict
    [Documentation]    Returns a dictionary of sensor name and its ID for required sensor component.
    ...  if argument is cpu - (example:{'dcm0_cpu0': '0x41', 'dcm0_cpu1': '0x42', 'dcm1_cpu0': '0x43', 'dcm1_cpu1': '0x44', 'dcm2_cpu0': '0x45', 'dcm2_cpu1': '0x46')
    ...  if argument is Ambient - (example:{'Ambient_0_Temp': '0x59', 'Ambient_1_Temp': '0x5a', 'Ambient_2_Temp': '0x5b'})
    [Arguments]       ${sensor_component}
