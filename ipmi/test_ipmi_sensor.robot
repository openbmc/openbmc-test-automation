...
Get IPMI Sensor Reading
    [Documentation]  Get IPMI sensor readings as dictionary.
    [Arguments]  ${sensor_id}
...
Get DCMI Sensor Reading
    [Documentation]  Get DCMI sensor readings as dictionary.
    [Arguments]  ${sensor_id}
    ...
    FOR  ${line}  IN  @{sensor_lines}
        ${line}=  Set Variable  ${line.split(' | ')}
        ${sensor_key}=  Set Variable  ${line[0].split(':')[1].strip()}
        ${sensor_value}=  Set Variable  ${line[1].split()[0].strip()}
