...
Verify Standalone SOL Output From DCSCM Using SBC Over IPMI
    ...
    PTY.Send To PTY Process  sol_ipmi_console_session  \n
    PTY.Send To PTY Process  sol_ipmi_console_session  ~?
    ${help_output}=  PTY.Read PTY Process Output  sol_ipmi_console_session  timeout=8
    ...
    PTY.Send To PTY Process  sol_ipmi_console_session  \n
    PTY.Send To PTY Process  sol_ipmi_console_session  ~\x1a
    Sleep  1s
    PTY.Resume PTY Process  sol_ipmi_console_session
    ${z_output}=  PTY.Read PTY Process Output  sol_ipmi_console_session  timeout=3
    Log  ~^Z suspend/resume output: ${z_output}

    # Test ~^X – suspend ipmitool (no tty restore), then resume via SIGCONT.
    PTY.Send To PTY Process  sol_ipmi_console_session  \n
    PTY.Send To PTY Process  sol_ipmi_console_session  ~\x18
    Sleep  1s
    PTY.Resume PTY Process  sol_ipmi_console_session
    ${x_output}=  PTY.Read PTY Process Output  sol_ipmi_console_session  timeout=3
    ...
    PTY.Send To PTY Process  sol_ipmi_console_session  ~~
    PTY.Send To PTY Process  sol_ipmi_console_session  \n
    ${tilde_output}=  PTY.Read PTY Process Output  sol_ipmi_console_session  timeout=3
    ...
    ${tilde_output}=  PTY.Read PTY Process Output  sol_ipmi_console_session  timeout=3
    Log  ~. (literal tilde) output: ${tilde_output}
    PTY.Stop PTY Process  sol_ipmi_console_session  force=${True}
    Sleep  1s
...
Verify SOL Serial Console Configuration Via Redfish
    ...
    ${system_resp}=  Redfish.Get  ${REDFISH_SYSTEMS_URI}/${SYSTEM_ID}
...
Verify Standalone SOL Output From DCSCM Using SBC Over SSH
    ...
    [Teardown]  Test Teardown
    ${TS}=  Get Time  result_format=%Y%m%d_%H%M%S
    VAR  ${log_file}  ${SOL_SESSION_LOG}_ssh_${TS}.log
    ...
    ...  alias=sol_ssh_console_session  timeout=30

    # Handle password prompt and authenticate.
    PTY.Wait For PTY Process Output  sol_ssh_console_session  [Pp]assword:  timeout=15
    PTY.Send Line To PTY Process  sol_ssh_console_session  ${OPENBMC_PASSWORD}
    ...
    ${conn_output}=  PTY.Read PTY Process Output  sol_ssh_console_session  timeout=10
    ...
    PTY.Send To PTY Process  sol_ssh_console_session  \n
    ${console_output}=  PTY.Read PTY Process Output  sol_ssh_console_session  timeout=5
    Log  SOL SSH console output after newline: ${console_output}

    # Exit the SSH session cleanly.
    PTY.Stop PTY Process  sol_ssh_console_session  force=${True}
    Sleep  1s
    ${output}=  PTY.Get Pty Process Log  ${log_file}
...
Verify IPMI SOL Activate Function
    ...
    ${ipmi_cmd}=  Create IPMI Ext Command String  sol activate usesolkeepalive
    PTY.Start Sol Pty Session  ${ipmi_cmd}  ${log_file}
    ...  alias=sol_activation_session  startup_timeout=30
    ...
    ${result}=  Run Process  ${deactivate_cmd}  shell=True
    Should Be Equal As Integers  ${result.rc}  ${0}
    Log  Deactivate output: ${result.stdout}

    PTY.Stop PTY Process  sol_activation_session  force=${True}
    Sleep  1s
    ${output}=  PTY.Get Pty Process Log  ${log_file}
    Should Contain  ${output}  ${SOL_OPERATIONAL_MSG}
...
Verify IPMI SOL Terminate Function
    ...
    ${ipmi_cmd}=  Create IPMI Ext Command String  sol activate usesolkeepalive
    PTY.Start Sol Pty Session  ${ipmi_cmd}  ${log_file}
    ...  alias=sol_termination_session  startup_timeout=30
    ...
    PTY.Stop PTY Process  sol_termination_session  force=${True}
    Sleep  1s
    ${output}=  PTY.Get Pty Process Log  ${log_file}
    Should Contain  ${output}  ${SOL_OPERATIONAL_MSG}
    ...
    ${deactivate_cmd}=  Create IPMI Ext Command String  sol deactivate
    ${result2}=  Run Process  ${deactivate_cmd}  shell=True
    Should Be Equal As Integers  ${result.rc}  ${0}
    Log  Second deactivate output: ${result2.stdout}
    LOG  Second deactivate error: ${result2.stderr}
    Should Contain  ${result2.stderr}  ${SOL_DEACTIVATED_MSG}
...
Verify IPMI SOL Activate While Another Session Active
    ...
    ${TS}=  Get Time  result_format=%Y%m%d_%H%M%S
    VAR  ${log1}  ${SOL_SESSION_LOG}_${TS}.log
    VAR  ${log2}  ${SOL_SESSION2_LOG}_${TS}.log
    ${ipmi_cmd}=  Create IPMI Ext Command String  sol activate usesolkeepalive
    PTY.Start Sol Pty Session  ${ipmi_cmd}  ${log1}
    ...  alias=sol_primary_session  startup_timeout=30
    ...
    ${client2_result}=  Run Process  ${client2_cmd}  shell=True
    Log  Client-2 activation error: ${client2_result.stderr}
    Should Contain  ${client2_result.stderr}  ${SOL_ALREADY_ACTIVE_MSG}
    ...  msg=Expected '${SOL_ALREADY_ACTIVE_MSG}' but got: ${client2_result.stderr}

    # Verify no crash/hang - rc should not indicate a hang (-1).
    Should Not Be Equal As Integers  ${client2_result.rc}  ${-1}
    ...  msg=Client-2 SOL activate command hung or crashed (rc=${client2_result.rc})
    ...
    PTY.Stop PTY Process  sol_primary_session  force=${True}
    Sleep  1s
    ...
    ...  alias=sol_secondary_session  startup_timeout=30
    ...
    PTY.Stop PTY Process  sol_secondary_session  force=${True}
    Sleep  1s
...
Test Setup
    ...
    ${result}=  Run Process  which ipmitool  shell=True
    Should Be Equal As Integers  ${result.rc}  ${0}
