...
Set Power Cap Value Via IPMI And Verify Using Redfish
    ...
    [Setup]  Redfish.Login
    [Teardown]  Set Power Cap Value Via Redfish  ${initial_power_value}
    [Tags]  Set_Power_Cap_Value_Via_IPMI_And_Verify_Using_Redfish

    # Get initial, minimum & maximum power cap value.
    ...
    #Generate random power cap value.
    ${power_capacity}=  Evaluate  random.randint(${min_power_value}, ${max_power_value})  modules=random

    # Set power cap value via IPMI.
    Run Keyword  Run IPMI Standard Command  dcmi power set_limit limit ${power_capacity}

    #Verify power cap value with Redfish value.
    ${power_cap_value}=  Get System Power Cap Limit
    Should Be Equal  ${power_cap_value['SetPoint']}  ${power_capacity}
...
Set Power Cap Value Via Redfish
    ...
    # Description of argument(s):
    # power cap value    Power cap value which need to be set.
