...
Get BMC Subnet Mask
    ...
    ${ip_data}=  Get BMC IP Info
    # Example: ${ip_data}= [xx.xx.xx.xx/24, xx.xx.xx.xx.xx/22]
    ${length}=  Get Length  ${ip_data}
    ${prefix_length}=  Get Regexp Matches  ${ip_data[${length}-1]}  /\\d{2}
    ${netmask_prefix_length}=    Remove String  ${prefix_length[0]}  /
    # Example:  Get Netmask Address  22
    ${subnet_mask}=  Get Netmask Address  ${netmask_prefix_length}
    [Return]  ${subnet_mask}
