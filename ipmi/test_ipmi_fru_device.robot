...
Read Fru Device configuration
    ...
    [Tags]  Read_Fru_Device_configuration
    ...
    ${ipmi_fru}=  Get IPMI Fru Devices  ${ipmi_output}

    # Returns all the available fru dbus uri.
    ${dbus_fru_uri}=  Get DBUS URI List  ${BUSCTL_FRU}  ${FRU_DBUS_URL}
    # Returns all the fru device uri with special characters removed.
    ${dbus_fru_uri_list}=  Generate DBUS URI List  ${dbus_fru_uri}
    # Creates dictionary with serial number as key,
    # and corresponding fru device uri as value.
    Get DBUS Dictionary Of Fru Devices  ${dbus_fru_uri_list}  ${ipmi_fru}
    # Compare dbus dictionary each field,
    # with ipmi fru device fields for each fru device.
...
Verify Get FRU Inventory Area Info
    ...
    ${resp}=  Read Fru Data  ${fru_device_id}  ${read_write_offset}
    ...
    # Compare read fru data Count returned -- count is ‘1’ based,
    # with inventory area count.
...
Verify IPMI Write FRU Data
    [Documentation]  Verify write data in FRU and
    ...  compare data from read fru data command via IPMI.
    [Tags]  Verify_IPMI_Write_FRU_Data
    [Setup]  Default Fru Data
    ...
    Write Fru Data  ${fru_device_id}  ${read_write_offset}  ${write_data_prefixed}  ${write_data_length}
...
Verify IPMI Write FRU Data With BMC Reboot
    [Documentation]  Verify ipmi write data in FRU and compare data
    ...  from read fru data command after bmc reboot.
...
Get IPMI Fru Devices
    ...
    FOR  ${devices}  IN  @{output}
        ...
        FOR  ${entry}  IN  @{dev}
            ...
            ${fru_entry}=  Create Dict With Value  ${tmp}  ${entry1}  ${entry2}
        END
        ...
        Set To Dictionary  ${fru}  ${serial_number}  ${fru_entry}
    END

    [Return]  ${fru}


Get DBUS Dictionary Of Fru Devices
...
Get FRU Inventory Area Info
    [Documentation]  Execute ipmi Get FRU Inventory Area Info command.

    # IPMI Get FRU Inventory Area Info and returns FRU Inventory area size in bytes.
...
Read Fru Data
    [Documentation]  Read fru data ipmi command.
...
Write Fru Data
    [Documentation]  Write fru data ipmi command.
...
IPMI BMC Reboot
    [Documentation]  Reboot BMC via ipmi command.
...
Restore Default Fru Data
    ...
    Write Fru Data  ${fru_device_id}  ${read_write_offset}  ${fru_byte}  ${initial_frulength_hex}
