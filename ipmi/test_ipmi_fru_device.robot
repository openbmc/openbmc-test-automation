   ...
   # ipmi_fru    IPMI FRU device details.
    ...
    FOR  ${key}  ${ipmi_fru_value}  IN  &{ipmi_fru}
        ...
        FOR  ${subkeys}  IN  @{ipmi_fru_subkeys}
            ...
            ${paramter_name}=  Get From Dictionary  ${ipmi_dbus_name_mapping}  ${subkeys}
            ${dbus_data}=  Get Lines Containing String  ${dbus_resp}  ${paramter_name}
            ${dbus_value}=  Set Variable  ${dbus_data.split('"')[1].strip()}
            ${ipmi_response}=  Get From Dictionary  ${ipmi_fru_value}  ${subkeys}
            ${status}=  Run Keyword And Return Status  Should Contain  ${paramter_name}  DATE
            IF  ${status}
                ...
                ...  message=${paramter_name} Parameter value mismatch with IPMI and DBUS
            ELSE
