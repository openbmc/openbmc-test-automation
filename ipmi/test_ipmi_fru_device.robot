...
Get IPMI FRU Devices Data
    ...
    FOR    ${devices}    IN    @{output}
        ...
        FOR    ${device}    IN    @{dev}
            ${ipmi_fru_board_serial_status}=    Run Keyword And Return Status
            ...    Should Contain
            ...    ${device}
            ...    Board Serial
        ...
        IF    '${ipmi_fru_board_serial_status}' == 'True'
            ${serial_no}=    Get Lines Containing String    ${frudata}    Board Serial
        ELSE
            ${serial_no}=    Get Lines Containing String    ${frudata}    Product Serial
        END
...
Get DBUS Dictionary For FRU Devices
    ...
    FOR    ${fru}    IN    @{dbus_fru}
        ...
        ${dbus_fru_board_serial_status}=    Run Keyword And Return Status
        ...    Should Contain
        ...    ${dbus_output}
        ...    .BOARD_SERIAL
        ${dbus_fru_product_serial_status}=    Run Keyword And Return Status
        ...    Should Contain
        ...    ${dbus_output}
        ...    .PRODUCT_SERIAL
...
Create Dictionary For DBUS URI
    ...
    FOR    ${ipmi_fru_serial_no}    IN    @{ipmi_fru.keys()}
        IF    '${dbus_fru_board_serial_status}' == 'True'
            ${serial_no}=    Get Lines Containing String    ${dbus_output}    .BOARD_SERIAL
        ELSE
            ${serial_no}=    Get Lines Containing String    ${dbus_output}    .PRODUCT_SERIAL
        END
        ${serial_no}=    Split String    ${serial_no}    "
        ${dbus_serial_no}=    Set Variable    ${serial_no[1].strip()}
        ${serial_no_status}=    Run Keyword And Return Status
