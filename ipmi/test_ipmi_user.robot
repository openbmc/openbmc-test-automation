...
Verify Administrator And No Access Privilege For Different Channels
    ...
    [Setup]  Check Number Of Active Ethernet Channels
    [Teardown]  Run Keywords  FFDC On Test Case Fail  AND
    ...  Delete Created User  ${random_userid}
...
Check Number Of Active Ethernet Channels
    [Documentation]  Check active ethernet channels

    ${channel_number_list}=  Get Active Ethernet Channel List

    FOR  ${CHANNEL_NO}  IN  @{channel_number_list}
        ${SECONDARY_CHANNEL_NUMBER}=  Set Variable If  ${CHANNEL_NO} != ${CHANNEL_NUMBER}  ${CHANNEL_NO}
    END
