...
Verify IPMI User Summary
    ...
    ${enabled_user_count}=
    ...  Get Lines Containing String  ${output}  Enabled User Count
    ${maximum_ids}=  Get Lines Containing String  ${output}  Maximum IDs

    # Get user count via Redfish.
    ${resp}=  Redfish.Get  /redfish/v1/AccountService/Accounts/
    ${count}=  Get From Dictionary  ${resp.dict}  Members@odata.count
    ${count}=  Evaluate  ${count}-1

    @{val}=  Split String  ${enabled_user_count}
    ${val}=  Evaluate  ${val}[4]

    Should Be Equal  ${val}  ${count}
