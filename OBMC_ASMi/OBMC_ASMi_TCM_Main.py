...
def main(argv):
    ...
    for i in m_TestCase:
        i == "OBMC_ASMi"
        print "Test Case Name:" + l_Test_Case
    ...
    try:
      ...
      + str('-i ') + str(l_Test_Case) + " " + str(l_obmc_asmi_stop_at_failure) + \
      " -N OpenBMC_ASMi --dotted --outputdir " + str(l_Result) + " " + str(l_Test_Function)

      subprocess.check_call(l_scl_cmd_python27 + l_chdir_cmd + l_obmc_asmi_rf_command, shell=True)
    ...
    print "Total Test Execution Time for this Test Case Execution: " + str((endtime - starttime).seconds) + " seconds\n"


    if(l_obmc_asmi_rf_command == True):
        print 'Test Passed'
        sys.exit(l_obmc_asmi_rf_command)
    else:
        print 'Test Failed'
        sys.exit(l_obmc_asmi_rf_command)
